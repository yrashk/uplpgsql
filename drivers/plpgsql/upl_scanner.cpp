 ...
 *
 * Derived from PostgreSQL src/pl/plpgsql/src/pl_scanner.c; modifications are
...
IdentifierLookup uplpgsql_IdentifierLookup = IDENTIFIER_LOOKUP_NORMAL;
 ...
 * isn't a known variable name.  If uplpgsql_IdentifierLookup is DECLARE then
...
struct uplpgsql_yy_extra_type
{
	...
	/*
	 * Current token's length (corresponds to uplpgsql_yylval and
	 * uplpgsql_yylloc)
	 */
	int			uplpgsql_yyleng;

	/* Current token's code (corresponds to uplpgsql_yylval and uplpgsql_yylloc) */
	int			uplpgsql_yytoken;
	...
	/* State for uplpgsql_location_to_lineno() */
...
struct yyguts_t
{
	struct uplpgsql_yy_extra_type *yyextra_r;
};
 ...
 * is turned off by uplpgsql_IdentifierLookup, it is returned as
 * T_WORD or T_CWORD respectively, or as an unreserved keyword if it
 * matches one of those.
 */
int
uplpgsql_yylex(YYSTYPE *yylvalp, YYLTYPE *yyllocp, yyscan_t yyscanner)
{
	int			tok1;
	TokenAuxData aux1;
	int			kwnum;
	...
	if (tok1 == IDENT || tok1 == PARAM)
	{
		...
		if (tok2 == '.')
		{
			...
			if (tok3 == IDENT)
			{
				...
				if (tok4 == '.')
				{
					...
					if (tok5 == IDENT)
					{
						if (uplpgsql_parse_tripword(aux1.lval.str,
												   aux3.lval.str,
												   aux5.lval.str,
												   &aux1.lval.wdatum,
												   &aux1.lval.cword))
					...
					else
					{
						...
						if (uplpgsql_parse_dblword(aux1.lval.str,
												  aux3.lval.str,
												  &aux1.lval.wdatum,
												  &aux1.lval.cword))
							tok1 = T_DATUM;
			...
			else
			{
				...
				if (uplpgsql_parse_word(aux1.lval.str,
		...
		else
		{
			 ...
			 *
			 * Note: we must call uplpgsql_parse_word even if we don't want to
			 * do variable lookup, because it sets up aux1.lval.word for the
			 * non-variable cases.
			 */
			if (uplpgsql_parse_word(aux1.lval.str,
								   yyextra->core_yy_extra.scanbuf + aux1.lloc,
								   (!AT_STMT_START(yyextra->uplpgsql_yytoken) ||
	...
	else
	{
		 ...
		 * uplpgsql_yytoken in the is-this-start-of-statement test above.
		 */
	}

	*yylvalp = aux1.lval;
	*yyllocp = aux1.lloc;
	yyextra->uplpgsql_yyleng = aux1.leng;
	yyextra->uplpgsql_yytoken = tok1;
	return tok1;
}

/*
 * Return the length of the token last returned by uplpgsql_yylex().
 *
 * In the case of compound tokens, the length includes all the parts.
 */
int
uplpgsql_token_length(yyscan_t yyscanner)
{
	return yyextra->uplpgsql_yyleng;
}
...
/*
 * Push back a single token to be re-read by next uplpgsql_yylex() call.
 *
 ...
 */
void
uplpgsql_push_back_token(int token, YYSTYPE *yylvalp, YYLTYPE *yyllocp, yyscan_t yyscanner)
{
	TokenAuxData auxdata;

	auxdata.lval = *yylvalp;
	auxdata.lloc = *yyllocp;
	auxdata.leng = yyextra->uplpgsql_yyleng;
 ...
 */
bool
uplpgsql_token_is_unreserved_keyword(int token)
{
	int			i;
 ...
 */
void
uplpgsql_append_source_text(StringInfo buf,
 ...
 */
int
uplpgsql_peek(yyscan_t yyscanner)
{
	int			tok1;
	TokenAuxData aux1;
 ...
 */
void
uplpgsql_peek2(int *tok1_p, int *tok2_p, int *tok1_loc, int *tok2_loc, yyscan_t yyscanner)
{
	int			tok1,
				tok2;
	TokenAuxData aux1,
 ...
 */
int
uplpgsql_scanner_errposition(int location, yyscan_t yyscanner)
{
	int			pos;
 ...
 * last returned by uplpgsql_yylex()).
 ...
 */
void
uplpgsql_yyerror(YYLTYPE *yyllocp, UPLpgSQL_stmt_block **uplpgsql_parse_result_p, yyscan_t yyscanner, const char *message)
{
	...
	if (*yytext == '\0')
	{
		...
		/* translator: %s is typically the translation of "syntax error" */
				 ...
				 uplpgsql_scanner_errposition(*yyllocp, yyscanner)));
	}
	else
	{
		/*
		 ...
		 * care about that.
		 */
		yytext[yyextra->uplpgsql_yyleng] = '\0';
 ...
 */
int
uplpgsql_location_to_lineno(int location, yyscan_t yyscanner)
{
	const char *loc;
...
/* initialize or reset the state for uplpgsql_location_to_lineno */
static void
...
int
uplpgsql_latest_lineno(yyscan_t yyscanner)
{
	return yyextra->cur_line_num;
}
 ...
 *
 * Note: the passed "str" must remain valid until uplpgsql_scanner_finish().
 * Although it is not fed directly to flex, we need the original string
 * to cite in error messages.
 */
yyscan_t
uplpgsql_scanner_init(const char *str)
{
	yyscan_t	yyscanner;
	struct uplpgsql_yy_extra_type *yyext = palloc0_object(struct uplpgsql_yy_extra_type);
...
/*
 * Called after parsing is done to clean up after uplpgsql_scanner_init()
 */
void
uplpgsql_scanner_finish(yyscan_t yyscanner)
{
	/* release storage */
