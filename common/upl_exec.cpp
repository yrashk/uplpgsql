 ...
 *
 * Derived from PostgreSQL src/pl/plpgsql/src/pl_exec.c; modifications are
 ...
 * uplpgsql_inline_handler().  DO blocks still use the simple_econtext_stack,
 ...
 * EState will be cleaned up when we get back to uplpgsql_inline_handler.  This
 ...
 *
 * cast_expr_hash entries (of type uplpgsql_CastExprHashEntry) hold compiled
 ...
 * uplpgsql_CastHashEntry) containing evaluation state trees for these
 ...
 * LOOP_RC_PROCESSING will do nothing except reset "rc" to UPLPGSQL_RC_OK.
 *
...
#define LOOP_RC_PROCESSING(looplabel, exit_action) \
	...
	else if (rc == UPLPGSQL_RC_CONTINUE) \
	{ \
		if (estate->exitlabel == NULL) \
		{ \
 ...
 ************************************************************/
static void coerce_function_result_tuple(UPLpgSQL_execstate *estate,
										 TupleDesc tupdesc);
static void uplpgsql_exec_error_callback(void *arg);
static void copy_uplpgsql_datums(UPLpgSQL_execstate *estate,
								UPLpgSQL_function *func);
static void uplpgsql_fulfill_promise(UPLpgSQL_execstate *estate,
									UPLpgSQL_var *var);
MemoryContext get_stmt_mcontext(UPLpgSQL_execstate *estate);
static void push_stmt_mcontext(UPLpgSQL_execstate *estate);
void pop_stmt_mcontext(UPLpgSQL_execstate *estate);

static int	exec_toplevel_block(UPLpgSQL_execstate *estate,
								UPLpgSQL_stmt_block *block);
int	exec_stmt_block(UPLpgSQL_execstate *estate,
							UPLpgSQL_stmt_block *block);
static int	exec_stmts(UPLpgSQL_execstate *estate,
					   List *stmts);
static int	exec_stmt_assign(UPLpgSQL_execstate *estate,
							 UPLpgSQL_stmt_assign *stmt);
int	exec_stmt_perform(UPLpgSQL_execstate *estate,
							  UPLpgSQL_stmt_perform *stmt);
int	exec_stmt_call(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_call *stmt);
int	exec_stmt_getdiag(UPLpgSQL_execstate *estate,
							  UPLpgSQL_stmt_getdiag *stmt);
static int	exec_stmt_if(UPLpgSQL_execstate *estate,
						 UPLpgSQL_stmt_if *stmt);
static int	exec_stmt_case(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_case *stmt);
static int	exec_stmt_loop(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_loop *stmt);
static int	exec_stmt_while(UPLpgSQL_execstate *estate,
							UPLpgSQL_stmt_while *stmt);
int	exec_stmt_fori(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_fori *stmt);
static int	exec_stmt_fors(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_fors *stmt);
static int	exec_stmt_forc(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_forc *stmt);
int	exec_stmt_foreach_a(UPLpgSQL_execstate *estate,
								UPLpgSQL_stmt_foreach_a *stmt);
int	exec_stmt_open(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_open *stmt);
int	exec_stmt_fetch(UPLpgSQL_execstate *estate,
							UPLpgSQL_stmt_fetch *stmt);
int	exec_stmt_close(UPLpgSQL_execstate *estate,
							UPLpgSQL_stmt_close *stmt);
static int	exec_stmt_exit(UPLpgSQL_execstate *estate,
						   UPLpgSQL_stmt_exit *stmt);
int	exec_stmt_return(UPLpgSQL_execstate *estate,
							 UPLpgSQL_stmt_return *stmt);
int	exec_stmt_return_next(UPLpgSQL_execstate *estate,
								  UPLpgSQL_stmt_return_next *stmt);
int	exec_stmt_return_query(UPLpgSQL_execstate *estate,
								   UPLpgSQL_stmt_return_query *stmt);
int	exec_stmt_raise(UPLpgSQL_execstate *estate,
							UPLpgSQL_stmt_raise *stmt);
int	exec_stmt_assert(UPLpgSQL_execstate *estate,
							 UPLpgSQL_stmt_assert *stmt);
int	exec_stmt_execsql(UPLpgSQL_execstate *estate,
							  UPLpgSQL_stmt_execsql *stmt);
int	exec_stmt_dynexecute(UPLpgSQL_execstate *estate,
								 UPLpgSQL_stmt_dynexecute *stmt);
int	exec_stmt_dynfors(UPLpgSQL_execstate *estate,
							  UPLpgSQL_stmt_dynfors *stmt);
int	exec_stmt_commit(UPLpgSQL_execstate *estate,
							 UPLpgSQL_stmt_commit *stmt);
int	exec_stmt_rollback(UPLpgSQL_execstate *estate,
							   UPLpgSQL_stmt_rollback *stmt);

void uplpgsql_estate_setup(UPLpgSQL_execstate *estate,
								 UPLpgSQL_function *func,
								 ReturnSetInfo *rsi,
								 EState *simple_eval_estate,
								 ResourceOwner simple_eval_resowner);
void exec_eval_cleanup(UPLpgSQL_execstate *estate);

static void exec_prepare_plan(UPLpgSQL_execstate *estate,
							  UPLpgSQL_expr *expr, int cursorOptions);
static void exec_simple_check_plan(UPLpgSQL_execstate *estate, UPLpgSQL_expr *expr);
static bool exec_is_simple_query(UPLpgSQL_expr *expr);
static void exec_save_simple_expr(UPLpgSQL_expr *expr, CachedPlan *cplan);
static void exec_check_rw_parameter(UPLpgSQL_expr *expr, int paramid);
static bool count_param_references(Node *node,
								   count_param_references_context *context);
static void exec_check_assignable(UPLpgSQL_execstate *estate, int dno);
static bool exec_eval_simple_expr(UPLpgSQL_execstate *estate,
								  UPLpgSQL_expr *expr,
								  Datum *result,
								  bool *isNull,
								  Oid *rettype,
...
void exec_assign_expr(UPLpgSQL_execstate *estate,
							 UPLpgSQL_datum *target,
							 UPLpgSQL_expr *expr);
static void exec_assign_c_string(UPLpgSQL_execstate *estate,
								 UPLpgSQL_datum *target,
								 const char *str);
void exec_assign_value(UPLpgSQL_execstate *estate,
							  UPLpgSQL_datum *target,
...
void exec_eval_datum(UPLpgSQL_execstate *estate,
							...
							Datum *value,
							bool *isnull);
int	exec_eval_integer(UPLpgSQL_execstate *estate,
							  UPLpgSQL_expr *expr,
							  bool *isNull);
bool exec_eval_boolean(UPLpgSQL_execstate *estate,
							  UPLpgSQL_expr *expr,
							  bool *isNull);
Datum exec_eval_expr(UPLpgSQL_execstate *estate,
							UPLpgSQL_expr *expr,
							bool *isNull,
							Oid *rettype,
							int32 *rettypmod);
int	exec_run_select(UPLpgSQL_execstate *estate,
							UPLpgSQL_expr *expr, long maxtuples, Portal *portalP);
static int	exec_for_query(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_forq *stmt,
						   Portal portal, bool prefetch_ok);
static ParamListInfo setup_param_list(UPLpgSQL_execstate *estate,
									  UPLpgSQL_expr *expr);
static ParamExternData *uplpgsql_param_fetch(ParamListInfo params,
											int paramid, bool speculative,
											ParamExternData *prm);
static void uplpgsql_param_compile(ParamListInfo params, Param *param,
								  ExprState *state,
								  Datum *resv, bool *resnull);
static void uplpgsql_param_eval_var_check(ExprState *state, ExprEvalStep *op,
										 ExprContext *econtext);
static void uplpgsql_param_eval_var_transfer(ExprState *state, ExprEvalStep *op,
											ExprContext *econtext);
static void uplpgsql_param_eval_var(ExprState *state, ExprEvalStep *op,
								   ExprContext *econtext);
static void uplpgsql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
									  ExprContext *econtext);
static void uplpgsql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
										ExprContext *econtext);
static void uplpgsql_param_eval_generic(ExprState *state, ExprEvalStep *op,
									   ExprContext *econtext);
static void uplpgsql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
										  ExprContext *econtext);
void exec_move_row(UPLpgSQL_execstate *estate,
						  UPLpgSQL_variable *target,
						  HeapTuple tup, TupleDesc tupdesc);
static void revalidate_rectypeid(UPLpgSQL_rec *rec);
static ExpandedRecordHeader *make_expanded_record_for_rec(UPLpgSQL_execstate *estate,
														  UPLpgSQL_rec *rec,
														  TupleDesc srctupdesc,
														  ExpandedRecordHeader *srcerh);
static void exec_move_row_from_fields(UPLpgSQL_execstate *estate,
									  UPLpgSQL_variable *target,
									  ExpandedRecordHeader *newerh,
									  Datum *values, bool *nulls,
									  TupleDesc tupdesc);
static bool compatible_tupdescs(TupleDesc src_tupdesc, TupleDesc dst_tupdesc);
static HeapTuple make_tuple_from_row(UPLpgSQL_execstate *estate,
									 UPLpgSQL_row *row,
									 TupleDesc tupdesc);
static TupleDesc deconstruct_composite_datum(Datum value,
											 HeapTupleData *tmptup);
static void exec_move_row_from_datum(UPLpgSQL_execstate *estate,
									 UPLpgSQL_variable *target,
									 Datum value);
static void instantiate_empty_record_variable(UPLpgSQL_execstate *estate,
											  UPLpgSQL_rec *rec);
static char *convert_value_to_string(UPLpgSQL_execstate *estate,
									 Datum value, Oid valtype);
static inline Datum exec_cast_value(UPLpgSQL_execstate *estate,
									Datum value, bool *isnull,
									Oid valtype, int32 valtypmod,
									Oid reqtype, int32 reqtypmod);
static Datum do_cast_value(UPLpgSQL_execstate *estate,
						   Datum value, bool *isnull,
						   Oid valtype, int32 valtypmod,
						   Oid reqtype, int32 reqtypmod);
static uplpgsql_CastHashEntry *get_cast_hashentry(UPLpgSQL_execstate *estate,
												 Oid srctype, int32 srctypmod,
												 Oid dsttype, int32 dsttypmod);
static void exec_init_tuple_store(UPLpgSQL_execstate *estate);
void exec_set_found(UPLpgSQL_execstate *estate, bool state);
void uplpgsql_create_econtext(UPLpgSQL_execstate *estate);
void uplpgsql_destroy_econtext(UPLpgSQL_execstate *estate);
void assign_simple_var(UPLpgSQL_execstate *estate, UPLpgSQL_var *var,
							  Datum newvalue, bool isnull, bool freeable);
void assign_text_var(UPLpgSQL_execstate *estate, UPLpgSQL_var *var,
							const char *str);
static void assign_record_var(UPLpgSQL_execstate *estate, UPLpgSQL_rec *rec,
							  ExpandedRecordHeader *erh);
static ParamListInfo exec_eval_using_params(UPLpgSQL_execstate *estate,
											List *params);
Portal exec_dynquery_with_params(UPLpgSQL_execstate *estate,
										UPLpgSQL_expr *dynquery, List *params,
										const char *portalname, int cursorOptions);
static char *format_expr_params(UPLpgSQL_execstate *estate,
								const UPLpgSQL_expr *expr);
static char *format_preparedparamsdata(UPLpgSQL_execstate *estate,
									   ParamListInfo paramLI);
static UPLpgSQL_variable *make_callstmt_target(UPLpgSQL_execstate *estate,
											  UPLpgSQL_expr *expr);


/* ----------
 * uplpgsql_exec_function	Called by the call handler for
 *				function execution.
 *
 ...
 */
Datum
uplpgsql_exec_function(UPLpgSQL_function *func, FunctionCallInfo fcinfo,
					  EState *simple_eval_estate,
					  ResourceOwner simple_eval_resowner,
					  ResourceOwner procedure_resowner,
					  bool atomic)
{
	UPLpgSQL_execstate estate;
	 ...
	 */
	uplpgsql_estate_setup(&estate, func, (ReturnSetInfo *) fcinfo->resultinfo,
	 ...
	 */
	plerrcontext.callback = uplpgsql_exec_error_callback;
	...
	copy_uplpgsql_datums(&estate, func);
	...
	for (i = 0; i < func->fn_nargs; i++)
	{
		...
		switch (estate.datums[n]->dtype)
		{
			case UPLPGSQL_DTYPE_VAR:
				{
					UPLpgSQL_var *var = (UPLpgSQL_var *) estate.datums[n];
			...
			case UPLPGSQL_DTYPE_REC:
				{
					UPLpgSQL_rec *rec = (UPLpgSQL_rec *) estate.datums[n];

					if (!fcinfo->args[i].isnull)
					{
						/* Assign row value from composite datum */
						exec_move_row_from_datum(&estate,
												 (UPLpgSQL_variable *) rec,
												 fcinfo->args[i].value);
					}
					else
					{
						/* If arg is null, set variable to null */
						exec_move_row(&estate, (UPLpgSQL_variable *) rec,
									  NULL, NULL);
					}
	 ...
	 */
	if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->func_beg)
		((*uplpgsql_plugin_ptr)->func_beg) (&estate, func);
	...
	else if (!estate.retisnull)
	{
		...
		if (estate.retistuple)
		{
			...
			else
			{
				 ...
				 * is in the UPLpgSQL_function struct, and that's too
				 * long-lived: composite types could change during the
				 * existence of a UPLpgSQL_function.
				 */
				Oid			resultTypeId;
				TupleDesc	tupdesc;
	 ...
	 * Let the instrumentation plugin peek at this function
	 */
	if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->func_end)
		((*uplpgsql_plugin_ptr)->func_end) (&estate, func);

	/* Clean up any leftover temporary memory */
	uplpgsql_destroy_econtext(&estate);
	exec_eval_cleanup(&estate);
...
/* ----------------------------------------------------------------
 * uplpgsql_exec_function_jit
 *
 * Execute a PL/pgSQL function via its JIT-compiled native code.
 * This mirrors uplpgsql_exec_function() exactly — same estate setup,
 * argument handling, and return-value processing — but calls the
 * JIT'd function instead of exec_toplevel_block().
 * ----------------------------------------------------------------
 */
Datum
uplpgsql_exec_function_jit(UPLpgSQL_function *func, FunctionCallInfo fcinfo,
						   EState *simple_eval_estate,
						   ResourceOwner simple_eval_resowner,
						   ResourceOwner procedure_resowner,
						   bool atomic,
...
{
	UPLpgSQL_execstate estate;
	UPLpgSQL_exec_state jit_estate;
	ErrorContextCallback plerrcontext;
	int			i;
	int			rc;
			...
			case UPLPGSQL_DTYPE_VAR:
				{
					...
					if (!var->isnull && var->datatype->typlen == -1)
					{
						...
						{
							/* R/O pointer, keep as-is until assigned to */
						}
						else if (var->datatype->typisarray)
						{
							assign_simple_var(&estate, var,
	...
	/*
	 * SQL/PSM: If the function's top-level block declares a SQLSTATE
	 * variable, activate it so runtime helpers (FETCH etc.) can update it.
	 */
	if (func->action->sqlstate_varno >= 0)
		estate.sqlstate_varno = func->action->sqlstate_varno;

	/*
	 * Set up JIT execution state and call the native function
	 */
	estate.err_text = NULL;
	memset(&jit_estate, 0, sizeof(jit_estate));
	jit_estate.uplpgsql_estate = &estate;

	rc = jit_func(&jit_estate);

	/*
	 * Return values are set in estate by exec_stmt_return (called via
	 * uplpgsql_rt_exec_return). No need to copy from jit_estate since
	 * the runtime helper writes directly to the PL/pgSQL estate.
	 *
	 * Clear err_stmt to match exec_stmt_block's behavior: the block is
	 * complete, so error context should no longer reference a statement.
	 * Without this, errors during return value casting would incorrectly
	 * include the RETURN statement's line number.
	 */
...
/* ----------------------------------------------------------------
 * uplpgsql_exec_trigger_jit
 *
 * Execute a trigger function via its JIT-compiled native code.
 * This mirrors uplpgsql_exec_trigger() exactly — same OLD/NEW setup,
 * and return-value processing — but calls the JIT'd function instead
 * of exec_toplevel_block().
 * ----------------------------------------------------------------
 */
HeapTuple
uplpgsql_exec_trigger_jit(UPLpgSQL_function *func, TriggerData *trigdata,
						  uplpgsql_jit_func jit_func)
{
	UPLpgSQL_execstate estate;
	UPLpgSQL_exec_state jit_estate;
	 ...
	 * Setup the execution state
	 */
	uplpgsql_estate_setup(&estate, func, NULL, NULL, NULL);
	...
	rec_new = (UPLpgSQL_rec *) (estate.datums[func->new_varno]);
	rec_old = (UPLpgSQL_rec *) (estate.datums[func->old_varno]);
	...
	/* SQL/PSM: activate SQLSTATE variable for trigger JIT path */
	if (func->action->sqlstate_varno >= 0)
		estate.sqlstate_varno = func->action->sqlstate_varno;
	...
	/* Clear err_stmt to match exec_stmt_block's post-block behavior */
	estate.err_stmt = NULL;
...
/*
 * Helper for uplpgsql_exec_function: coerce composite result to the specified
 ...
 */
static void
coerce_function_result_tuple(UPLpgSQL_execstate *estate, TupleDesc tupdesc)
{
	HeapTuple	rettup;
	TupleDesc	retdesc;
	TupleConversionMap *tupmap;
...
/* ----------
 * uplpgsql_exec_trigger		Called by the call handler for
 *				trigger execution.
 * ----------
 */
HeapTuple
uplpgsql_exec_trigger(UPLpgSQL_function *func,
...
/* ----------
 * uplpgsql_exec_event_trigger		Called by the call handler for
 *				event trigger execution.
 * ----------
 */
void
uplpgsql_exec_event_trigger(UPLpgSQL_function *func, EventTriggerData *trigdata)
{
	UPLpgSQL_execstate estate;
	ErrorContextCallback plerrcontext;
	int			rc;
 ...
 */
static void
uplpgsql_exec_error_callback(void *arg)
{
	UPLpgSQL_execstate *estate = (UPLpgSQL_execstate *) arg;
	int			err_lineno;
	...
	else if (estate->err_stmt != NULL && err_lineno > 0)
	{
		...
		errcontext("PL/pgSQL function %s line %d at %s",
				   estate->func->fn_signature,
				   err_lineno,
				   uplpgsql_stmt_typename(estate->err_stmt));
	}
	else
 ...
 */
static void
copy_uplpgsql_datums(UPLpgSQL_execstate *estate,
					UPLpgSQL_function *func)
{
	int			ndatums = estate->ndatums;
	UPLpgSQL_datum **indatums;
	UPLpgSQL_datum **outdatums;
	char	   *workspace;
	char	   *ws_next;
	int			i;

	/* Allocate local datum-pointer array */
	estate->datums = palloc_array(UPLpgSQL_datum *, ndatums);
	...
	for (i = 0; i < ndatums; i++)
	{
		UPLpgSQL_datum *indatum = indatums[i];
		UPLpgSQL_datum *outdatum;

		/* This must agree with uplpgsql_finish_datums on what is copiable */
		switch (indatum->dtype)
		{
			case UPLPGSQL_DTYPE_VAR:
			case UPLPGSQL_DTYPE_PROMISE:
				outdatum = (UPLpgSQL_datum *) ws_next;
				memcpy(outdatum, indatum, sizeof(UPLpgSQL_var));
				ws_next += MAXALIGN(sizeof(UPLpgSQL_var));
				break;

			case UPLPGSQL_DTYPE_REC:
				outdatum = (UPLpgSQL_datum *) ws_next;
				memcpy(outdatum, indatum, sizeof(UPLpgSQL_rec));
				ws_next += MAXALIGN(sizeof(UPLpgSQL_rec));
				break;
 ...
 */
static void
uplpgsql_fulfill_promise(UPLpgSQL_execstate *estate,
						UPLpgSQL_var *var)
{
	MemoryContext oldcontext;

	if (var->promise == UPLPGSQL_PROMISE_NONE)
	...
	switch (var->promise)
	{
		...
		case UPLPGSQL_PROMISE_TG_LEVEL:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
		...
		case UPLPGSQL_PROMISE_TG_RELID:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
		...
		case UPLPGSQL_PROMISE_TG_TABLE_NAME:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
		...
		case UPLPGSQL_PROMISE_TG_TABLE_SCHEMA:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
							  DirectFunctionCall1(namein,
		...
		case UPLPGSQL_PROMISE_TG_NARGS:
			if (estate->trigdata == NULL)
				elog(ERROR, "trigger promise is not in a trigger function");
			assign_simple_var(estate, var,
		...
		case UPLPGSQL_PROMISE_TG_EVENT:
 ...
 */
MemoryContext
get_stmt_mcontext(UPLpgSQL_execstate *estate)
{
 ...
 */
static void
push_stmt_mcontext(UPLpgSQL_execstate *estate)
{
 ...
 */
void
pop_stmt_mcontext(UPLpgSQL_execstate *estate)
{
 ...
 */
bool
exception_matches_conditions(ErrorData *edata, UPLpgSQL_condition *cond)
{
	for (; cond != NULL; cond = cond->next)
	{
		 ...
		 * explicitly.
		 */
		if (sqlerrstate == UPLPGSQL_OTHERS)
		{
 ...
 */
static int
exec_toplevel_block(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_block *block)
{
	int			rc;

	estate->err_stmt = (UPLpgSQL_stmt *) block;

	/* Let the plugin know that we are about to execute this statement */
	if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->stmt_beg)
		((*uplpgsql_plugin_ptr)->stmt_beg) (estate, (UPLpgSQL_stmt *) block);
	...
	if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->stmt_end)
		((*uplpgsql_plugin_ptr)->stmt_end) (estate, (UPLpgSQL_stmt *) block);
 ...
 */
int
exec_stmt_block(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_block *block)
{
	volatile int rc = -1;
	int			i;
	int			old_sqlstate_varno = estate->sqlstate_varno;
	...
	for (i = 0; i < block->n_initvars; i++)
	{
		...
		UPLpgSQL_datum *datum = estate->datums[n];

		/*
		 * The set of dtypes handled here must match uplpgsql_add_initdatums().
		 *
		 ...
		 * UPLpgSQL_datum to UPLpgSQL_variable.
		 */
		estate->err_var = (UPLpgSQL_variable *) datum;
			...
			case UPLPGSQL_DTYPE_VAR:
				{
					...
					{
						exec_assign_expr(estate, (UPLpgSQL_datum *) var,
										 var->default_val);
					}
				}
				break;
			...
			case UPLPGSQL_DTYPE_REC:
				{
					...
					{
						exec_assign_expr(estate, (UPLpgSQL_datum *) rec,
										 rec->default_val);
					}
				}
				break;
	...
	/*
	 * SQL/PSM: If this block declares a SQLSTATE variable, activate it
	 * and initialize to '00000' (SUCCESSFUL_COMPLETION).
	 */
	if (block->sqlstate_varno >= 0 &&
		estate->sqlstate_varno != block->sqlstate_varno)
	{
		estate->sqlstate_varno = block->sqlstate_varno;
		estate->sqlerrcode = ERRCODE_SUCCESSFUL_COMPLETION;
		uplpgsql_set_sqlstate(estate);
	}
		...
		PG_CATCH();
		{
			...
			foreach(e, block->exceptions->exc_list)
			{
				...
				if (exception_matches_conditions(edata, exception->conditions))
				{
					...
					/*
					 * SQL/PSM: Update user-declared SQLSTATE variable
					 * with the caught error's SQLSTATE code.
					 */
					if (estate->sqlstate_varno >= 0)
					{
	...
	/* Restore previous SQLSTATE variable scope */
	...
	{
		case UPLPGSQL_RC_OK:
		case UPLPGSQL_RC_RETURN:
		case UPLPGSQL_RC_CONTINUE:
			return rc;

		case UPLPGSQL_RC_EXIT:
			if (estate->exitlabel == NULL)
				return UPLPGSQL_RC_EXIT;
			if (block->label == NULL)
				return UPLPGSQL_RC_EXIT;
...
exec_stmts(UPLpgSQL_execstate *estate, List *stmts)
{
	...
	foreach(s, stmts)
	{
		...
		/* Let the plugin know that we are about to execute this statement */
		if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->stmt_beg)
			((*uplpgsql_plugin_ptr)->stmt_beg) (estate, stmt);
		...
		switch (stmt->cmd_type)
		{
			case UPLPGSQL_STMT_BLOCK:
				rc = exec_stmt_block(estate, (UPLpgSQL_stmt_block *) stmt);
				break;

			case UPLPGSQL_STMT_ASSIGN:
				rc = exec_stmt_assign(estate, (UPLpgSQL_stmt_assign *) stmt);
				break;

			case UPLPGSQL_STMT_PERFORM:
				rc = exec_stmt_perform(estate, (UPLpgSQL_stmt_perform *) stmt);
				break;

			case UPLPGSQL_STMT_CALL:
				rc = exec_stmt_call(estate, (UPLpgSQL_stmt_call *) stmt);
				break;

			case UPLPGSQL_STMT_GETDIAG:
				rc = exec_stmt_getdiag(estate, (UPLpgSQL_stmt_getdiag *) stmt);
				break;

			case UPLPGSQL_STMT_IF:
				rc = exec_stmt_if(estate, (UPLpgSQL_stmt_if *) stmt);
				break;

			case UPLPGSQL_STMT_CASE:
				rc = exec_stmt_case(estate, (UPLpgSQL_stmt_case *) stmt);
				break;

			case UPLPGSQL_STMT_LOOP:
				rc = exec_stmt_loop(estate, (UPLpgSQL_stmt_loop *) stmt);
				break;

			case UPLPGSQL_STMT_WHILE:
				rc = exec_stmt_while(estate, (UPLpgSQL_stmt_while *) stmt);
				break;

			case UPLPGSQL_STMT_FORI:
				rc = exec_stmt_fori(estate, (UPLpgSQL_stmt_fori *) stmt);
				break;

			case UPLPGSQL_STMT_FORS:
				rc = exec_stmt_fors(estate, (UPLpgSQL_stmt_fors *) stmt);
				break;

			case UPLPGSQL_STMT_FORC:
				rc = exec_stmt_forc(estate, (UPLpgSQL_stmt_forc *) stmt);
				break;

			case UPLPGSQL_STMT_FOREACH_A:
				rc = exec_stmt_foreach_a(estate, (UPLpgSQL_stmt_foreach_a *) stmt);
				break;

			case UPLPGSQL_STMT_EXIT:
				rc = exec_stmt_exit(estate, (UPLpgSQL_stmt_exit *) stmt);
				break;

			case UPLPGSQL_STMT_RETURN:
				rc = exec_stmt_return(estate, (UPLpgSQL_stmt_return *) stmt);
				break;

			case UPLPGSQL_STMT_RETURN_NEXT:
				rc = exec_stmt_return_next(estate, (UPLpgSQL_stmt_return_next *) stmt);
				break;

			case UPLPGSQL_STMT_RETURN_QUERY:
				rc = exec_stmt_return_query(estate, (UPLpgSQL_stmt_return_query *) stmt);
				break;

			case UPLPGSQL_STMT_RAISE:
				rc = exec_stmt_raise(estate, (UPLpgSQL_stmt_raise *) stmt);
				break;

			case UPLPGSQL_STMT_ASSERT:
				rc = exec_stmt_assert(estate, (UPLpgSQL_stmt_assert *) stmt);
				break;

			case UPLPGSQL_STMT_EXECSQL:
				rc = exec_stmt_execsql(estate, (UPLpgSQL_stmt_execsql *) stmt);
				break;

			case UPLPGSQL_STMT_DYNEXECUTE:
				rc = exec_stmt_dynexecute(estate, (UPLpgSQL_stmt_dynexecute *) stmt);
				break;

			case UPLPGSQL_STMT_DYNFORS:
				rc = exec_stmt_dynfors(estate, (UPLpgSQL_stmt_dynfors *) stmt);
				break;

			case UPLPGSQL_STMT_OPEN:
				rc = exec_stmt_open(estate, (UPLpgSQL_stmt_open *) stmt);
				break;

			case UPLPGSQL_STMT_FETCH:
				rc = exec_stmt_fetch(estate, (UPLpgSQL_stmt_fetch *) stmt);
				break;

			case UPLPGSQL_STMT_CLOSE:
				rc = exec_stmt_close(estate, (UPLpgSQL_stmt_close *) stmt);
				break;

			case UPLPGSQL_STMT_COMMIT:
				rc = exec_stmt_commit(estate, (UPLpgSQL_stmt_commit *) stmt);
				break;

			case UPLPGSQL_STMT_ROLLBACK:
				rc = exec_stmt_rollback(estate, (UPLpgSQL_stmt_rollback *) stmt);
				break;
		...
		/* Let the plugin know that we have finished executing this statement */
		if (*uplpgsql_plugin_ptr && (*uplpgsql_plugin_ptr)->stmt_end)
			((*uplpgsql_plugin_ptr)->stmt_end) (estate, stmt);

		if (rc != UPLPGSQL_RC_OK)
		{
			estate->err_stmt = save_estmt;
			return rc;
		}
 ...
 */
static int
exec_stmt_assign(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_assign *stmt)
{
	Assert(stmt->varno >= 0);
 ...
 * exec_move_row() to do the assignments.
 */
static UPLpgSQL_variable *
make_callstmt_target(UPLpgSQL_execstate *estate, UPLpgSQL_expr *expr)
{
	CachedPlan *cplan;
	PlannedStmt *pstmt;
	CallStmt   *stmt;
 ...
 */
static int
exec_stmt_if(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_if *stmt)
{
	bool		value;
	bool		isnull;
	ListCell   *lc;
 ...
 */
static int
exec_stmt_case(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_case *stmt)
{
	...
	if (stmt->t_expr != NULL)
	{
		...
		t_var = (UPLpgSQL_var *) estate->datums[stmt->t_varno];
 ...
 */
static int
exec_stmt_loop(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_loop *stmt)
{
	int			rc = UPLPGSQL_RC_OK;
 ...
 */
static int
exec_stmt_while(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_while *stmt)
{
	int			rc = UPLPGSQL_RC_OK;
	...
	for (;;)
	{
		...
		/* WHILE: test before body.  REPEAT UNTIL: test after body. */
		if (stmt->test_at_top)
		{
			value = exec_eval_boolean(estate, stmt->cond, &isnull);
			exec_eval_cleanup(estate);
 ...
 */
int
exec_stmt_fori(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_fori *stmt)
{
	UPLpgSQL_var *var;
	Datum		value;
	bool		isnull;
	...
	var = (UPLpgSQL_var *) (estate->datums[stmt->var->dno]);
 ...
 */
static int
exec_stmt_fors(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_fors *stmt)
{
	Portal		portal;
	int			rc;
	 ...
	 * Execute the loop
	 */
	rc = exec_for_query(estate, (UPLpgSQL_stmt_forq *) stmt, portal, true);
 ...
 */
static int
exec_stmt_forc(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_forc *stmt)
{
	UPLpgSQL_var *curvar;
	 ...
	 * ----------
	 */
	curvar = (UPLpgSQL_var *) (estate->datums[stmt->curvar]);
	if (!curvar->isnull)
	{
		MemoryContext oldcontext;
	...
	if (stmt->argquery != NULL)
	{
		 ...
		 * ----------
		 */
		UPLpgSQL_stmt_execsql set_args;
		...
		set_args.cmd_type = UPLPGSQL_STMT_EXECSQL;
		set_args.lineno = stmt->lineno;
		set_args.sqlstmt = stmt->argquery;
		set_args.into = true;
		/* XXX historically this has not been STRICT */
		set_args.target = (UPLpgSQL_variable *)
			(estate->datums[curvar->cursor_explicit_argrow]);

		if (exec_stmt_execsql(estate, &set_args) != UPLPGSQL_RC_OK)
	 ...
	 */
	rc = exec_for_query(estate, (UPLpgSQL_stmt_forq *) stmt, portal, false);
...
/* ----------
 * exec_open_forc_cursor		Open the cursor for a FOR-cursor loop.
 *
 * This is the setup portion of exec_stmt_forc, factored out so that
 * JIT'd code can open the cursor at runtime while keeping the loop
 * control flow in native LLVM IR.
 * ----------
 */
Portal
exec_open_forc_cursor(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_forc *stmt)
{
	UPLpgSQL_var *curvar;
	MemoryContext stmt_mcontext = NULL;
	char	   *curname = NULL;
	...
	/*
	 * Pin the portal to prevent user statements from closing it mid-loop.
	 * The JIT loop will call exec_close_forc_cursor which unpins.
	 */
	PinPortal(portal);
...
/* ----------
 * exec_close_forc_cursor		Close the cursor for a FOR-cursor loop.
 *
 * Unpins the portal, closes it, and resets the cursor variable to NULL
 * if it was auto-assigned (i.e., the cursor variable name was originally
 * NULL and the portal name was auto-generated).
 * ----------
 */
void
exec_close_forc_cursor(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_forc *stmt,
					   Portal portal)
{
	UPLpgSQL_var *curvar;
	 ...
	 * reset it to NULL.  We detect this by checking if the cursor variable
	 * has a non-null text value — if it was originally NULL, we assigned
	 * the auto-generated name in exec_open_forc_cursor.
	 *
	 * Note: This is conservative.  The original code tracked curname==NULL
	 * through a local variable, but we don't have that luxury across calls.
	 * Resetting to NULL is always safe for auto-generated names.
	 */
	curvar = (UPLpgSQL_var *) (estate->datums[stmt->curvar]);
	(void) curvar;  /* reset handled by cursor lifecycle */
}
...
exec_stmt_foreach_a(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_foreach_a *stmt)
{
	...
	if (loop_var->dtype == UPLPGSQL_DTYPE_REC ||
		loop_var->dtype == UPLPGSQL_DTYPE_ROW)
	{
		/*
		...
		loop_var_elem_type = get_element_type(uplpgsql_exec_get_datum_type(estate,
																		  loop_var));
 ...
 */
static int
exec_stmt_exit(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_exit *stmt)
{
	/*
 ...
 */
int
exec_stmt_return(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_return *stmt)
{
	/*
	...
	if (stmt->retvarno >= 0)
	{
		UPLpgSQL_datum *retvar = estate->datums[stmt->retvarno];

		switch (retvar->dtype)
		{
			case UPLPGSQL_DTYPE_PROMISE:
				/* fulfill promise if needed, then handle like regular var */
				uplpgsql_fulfill_promise(estate, (UPLpgSQL_var *) retvar);

				pg_fallthrough;

			case UPLPGSQL_DTYPE_VAR:
				{
					UPLpgSQL_var *var = (UPLpgSQL_var *) retvar;
					...
					/*
					 * A UPLpgSQL_var could not be of composite type, so
					 ...
					 * is NULL.  (Note that uplpgsql_exec_trigger assumes that
					  ...
					  UPLpgSQL_stmt_return_next *stmt)
{
	...
	if (stmt->retvarno >= 0)
	{
		...
		switch (retvar->dtype)
		{
			...
			case UPLPGSQL_DTYPE_REC:
				{
					UPLpgSQL_rec *rec = (UPLpgSQL_rec *) retvar;
			...
			case UPLPGSQL_DTYPE_ROW:
				{
					UPLpgSQL_row *row = (UPLpgSQL_row *) retvar;
...
static void
exec_init_tuple_store(UPLpgSQL_execstate *estate)
{
	ReturnSetInfo *rsi = estate->rsi;
	MemoryContext oldcxt;
...
exec_stmt_raise(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_raise *stmt)
{
	...
	if (stmt->condname)
	{
		err_code = uplpgsql_recognize_err_condition(stmt->condname, true);
		...
		for (cp = stmt->message; *cp; cp++)
		{
			...
			if (cp[0] == '%')
			{
				...
				paramvalue = exec_eval_expr(estate,
											(UPLpgSQL_expr *) lfirst(current_param),
											&paramisnull,
											&paramtypeid,
											&paramtypmod);
	...
	foreach(lc, stmt->options)
	{
		...
		switch (opt->opt_type)
		{
			case UPLPGSQL_RAISEOPTION_ERRCODE:
				if (err_code)
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("RAISE option already specified: %s",
									"ERRCODE")));
				err_code = uplpgsql_recognize_err_condition(extval, true);
				condname = MemoryContextStrdup(stmt_mcontext, extval);
				break;
			case UPLPGSQL_RAISEOPTION_MESSAGE:
				SET_RAISE_OPTION_TEXT(err_message, "MESSAGE");
				break;
			case UPLPGSQL_RAISEOPTION_DETAIL:
				SET_RAISE_OPTION_TEXT(err_detail, "DETAIL");
				break;
			case UPLPGSQL_RAISEOPTION_HINT:
				SET_RAISE_OPTION_TEXT(err_hint, "HINT");
				break;
			case UPLPGSQL_RAISEOPTION_COLUMN:
				SET_RAISE_OPTION_TEXT(err_column, "COLUMN");
				break;
			case UPLPGSQL_RAISEOPTION_CONSTRAINT:
				SET_RAISE_OPTION_TEXT(err_constraint, "CONSTRAINT");
				break;
			case UPLPGSQL_RAISEOPTION_DATATYPE:
				SET_RAISE_OPTION_TEXT(err_datatype, "DATATYPE");
				break;
			case UPLPGSQL_RAISEOPTION_TABLE:
				SET_RAISE_OPTION_TEXT(err_table, "TABLE");
				break;
			case UPLPGSQL_RAISEOPTION_SCHEMA:
 ...
 */
int
exec_stmt_assert(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_assert *stmt)
{
	bool		value;
	bool		isnull;

	/* do nothing when asserts are not enabled */
	if (!uplpgsql_check_asserts)
		return UPLPGSQL_RC_OK;
 ...
 */
void
uplpgsql_estate_setup(UPLpgSQL_execstate *estate,
					 UPLpgSQL_function *func,
					 ReturnSetInfo *rsi,
					 EState *simple_eval_estate,
					 ResourceOwner simple_eval_resowner)
{
	...
	/* this link will be restored at exit from uplpgsql_call_handler */
	...
	estate->sqlstate_varno = -1;
	estate->sqlerrcode = ERRCODE_SUCCESSFUL_COMPLETION;
	estate->ndatums = func->ndatums;
	estate->datums = NULL;
	/* the datums array will be filled by copy_uplpgsql_datums() */
	...
	estate->paramLI->paramFetch = uplpgsql_param_fetch;
	estate->paramLI->paramFetchArg = estate;
	estate->paramLI->paramCompile = uplpgsql_param_compile;
	estate->paramLI->paramCompileArg = NULL;	/* not needed */
	estate->paramLI->parserSetup = (ParserSetupHook) uplpgsql_parser_setup;
	...
	{
		ctl.keysize = sizeof(uplpgsql_CastHashKey);
		ctl.entrysize = sizeof(uplpgsql_CastExprHashEntry);
	...
	if (simple_eval_estate)
	{
		...
		ctl.keysize = sizeof(uplpgsql_CastHashKey);
		ctl.entrysize = sizeof(uplpgsql_CastHashEntry);
	 ...
	 */
	if (*uplpgsql_plugin_ptr)
	{
		(*uplpgsql_plugin_ptr)->error_callback = uplpgsql_exec_error_callback;
		(*uplpgsql_plugin_ptr)->assign_expr = exec_assign_expr;
		(*uplpgsql_plugin_ptr)->assign_value = exec_assign_value;
		(*uplpgsql_plugin_ptr)->eval_datum = exec_eval_datum;
		(*uplpgsql_plugin_ptr)->cast_value = exec_cast_value;

		if ((*uplpgsql_plugin_ptr)->func_setup)
			((*uplpgsql_plugin_ptr)->func_setup) (estate, func);
	}
}
 ...
 */
void
exec_eval_cleanup(UPLpgSQL_execstate *estate)
{
 ...
 */
static void
exec_prepare_plan(UPLpgSQL_execstate *estate,
				  UPLpgSQL_expr *expr, int cursorOptions)
{
	SPIPlanPtr	plan;
				  ...
				  UPLpgSQL_stmt_execsql *stmt)
{
	...
	long		tcount;
	int			rc;
	UPLpgSQL_expr *expr = stmt->sqlstmt;
	int			too_many_rows_level = 0;

	if (uplpgsql_extra_errors & UPLPGSQL_XCHECK_TOOMANYROWS)
		too_many_rows_level = ERROR;
	else if (uplpgsql_extra_warnings & UPLPGSQL_XCHECK_TOOMANYROWS)
	...
	if (stmt->into)
	{
		...
		target = (UPLpgSQL_variable *) estate->datums[stmt->target->dno];
 ...
 */
int
exec_stmt_dynfors(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_dynfors *stmt)
{
	Portal		portal;
	int			rc;
...
exec_stmt_fetch(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_fetch *stmt)
{
	...
	/* SQL/PSM: Update SQLSTATE variable after FETCH/MOVE */
	if (estate->sqlstate_varno >= 0)
	{
		if (n == 0)
			estate->sqlerrcode = ERRCODE_NO_DATA;	/* '02000' NOT FOUND */
		else
			estate->sqlerrcode = ERRCODE_SUCCESSFUL_COMPLETION;
		uplpgsql_set_sqlstate(estate);
	}
 ...
 */
static void
exec_assign_c_string(UPLpgSQL_execstate *estate, UPLpgSQL_datum *target,
					 const char *str)
{
	text	   *value;
	MemoryContext oldcontext;
 ...
 */
void
exec_assign_value(UPLpgSQL_execstate *estate,
				  UPLpgSQL_datum *target,
				  Datum value, bool isNull,
				  Oid valtype, int32 valtypmod)
{
		...
		case UPLPGSQL_DTYPE_PROMISE:
			{
				 ...
				 */
				UPLpgSQL_var *var = (UPLpgSQL_var *) target;
				Datum		newvalue;
				...
				if (var->value != newvalue || var->isnull || isNull)
					...
					var->promise = UPLPGSQL_PROMISE_NONE;
				break;
			}
		...
		case UPLPGSQL_DTYPE_ROW:
			{
				 ...
				 */
				UPLpgSQL_row *row = (UPLpgSQL_row *) target;

				if (isNull)
				{
					/* If source is null, just assign nulls to the row */
					exec_move_row(estate, (UPLpgSQL_variable *) row,
								  NULL, NULL);
				}
				else
				{
					...
					exec_move_row_from_datum(estate, (UPLpgSQL_variable *) row,
											 value);
				}
				break;
			}
		...
		case UPLPGSQL_DTYPE_REC:
			{
				...
				else
				{
					...
					exec_move_row_from_datum(estate, (UPLpgSQL_variable *) rec,
											 value);
				}
				break;
			}
		...
		case UPLPGSQL_DTYPE_RECFIELD:
			{
				 ...
				 */
				UPLpgSQL_recfield *recfield = (UPLpgSQL_recfield *) target;
				UPLpgSQL_rec *rec;
				ExpandedRecordHeader *erh;

				rec = (UPLpgSQL_rec *) (estate->datums[recfield->recparentno]);
				erh = rec->erh;
...
/*
 * exec_eval_datum				Get current value of a UPLpgSQL_datum
 *
 * The type oid, typmod, value in Datum format, and null flag are returned.
 *
 * At present this doesn't handle UPLpgSQL_expr datums; that's not needed
				...
				bool *isnull)
{
	...
	switch (datum->dtype)
	{
		case UPLPGSQL_DTYPE_PROMISE:
			/* fulfill promise if needed, then handle like regular var */
			uplpgsql_fulfill_promise(estate, (UPLpgSQL_var *) datum);
		...
		case UPLPGSQL_DTYPE_ROW:
			{
				UPLpgSQL_row *row = (UPLpgSQL_row *) datum;
				HeapTuple	tup;
		...
		case UPLPGSQL_DTYPE_RECFIELD:
			{
				UPLpgSQL_recfield *recfield = (UPLpgSQL_recfield *) datum;
				UPLpgSQL_rec *rec;
				ExpandedRecordHeader *erh;
...
/*
 * uplpgsql_exec_get_datum_type				Get datatype of a UPLpgSQL_datum
 *
...
/*
 * uplpgsql_exec_get_datum_type_info			Get datatype etc of a UPLpgSQL_datum
 *
 * An extended version of uplpgsql_exec_get_datum_type, which also retrieves the
 ...
 */
void
uplpgsql_exec_get_datum_type_info(UPLpgSQL_execstate *estate,
								 UPLpgSQL_datum *datum,
 ...
 */
int
exec_eval_integer(UPLpgSQL_execstate *estate,
				  UPLpgSQL_expr *expr,
				  bool *isNull)
{
	Datum		exprdatum;
 ...
 */
bool
exec_eval_boolean(UPLpgSQL_execstate *estate,
				  UPLpgSQL_expr *expr,
				  bool *isNull)
{
	Datum		exprdatum;
 ...
 */
Datum
exec_eval_expr(UPLpgSQL_execstate *estate,
			   UPLpgSQL_expr *expr,
			   bool *isNull,
			   Oid *rettype,
			   int32 *rettypmod)
 ...
 */
int
exec_run_select(UPLpgSQL_execstate *estate,
				UPLpgSQL_expr *expr, long maxtuples, Portal *portalP)
{
	ParamListInfo paramLI;
	int			rc;
 ...
 */
static int
exec_for_query(UPLpgSQL_execstate *estate, UPLpgSQL_stmt_forq *stmt,
			   Portal portal, bool prefetch_ok)
{
	...
	var = (UPLpgSQL_variable *) estate->datums[stmt->var->dno];
	...
	while (n > 0)
	{
		...
		for (i = 0; i < n; i++)
		{
			 ...
			 */
			if (var->dtype == UPLPGSQL_DTYPE_REC)
			{
				UPLpgSQL_rec *rec = (UPLpgSQL_rec *) var;
 ...
 */
static bool
exec_eval_simple_expr(UPLpgSQL_execstate *estate,
					  UPLpgSQL_expr *expr,
					  Datum *result,
					  bool *isNull,
					  Oid *rettype,
					  int32 *rettypmod)
{
	...
	else
	{
		...
		expr->expr_rwopt = UPLPGSQL_RWOPT_UNKNOWN;
 ...
 * UPLpgSQL_exprs in this estate.  It contains no per-param data, just hook
 * functions, so it's effectively read-only for SPI.
 *
 * An exception from pure read-only-ness is that the parserSetupArg points
 * to the specific UPLpgSQL_expr being evaluated.  This is not an issue for
 ...
 * create a ParamListInfo struct for each UPLpgSQL_expr, but for the moment
 * that seems like a waste of memory.)
 */
static ParamListInfo
setup_param_list(UPLpgSQL_execstate *estate, UPLpgSQL_expr *expr)
{
	ParamListInfo paramLI;
...
/*
 * uplpgsql_param_fetch		paramFetch callback for dynamic parameter fetch
 *
...
uplpgsql_param_fetch(ParamListInfo params,
					int paramid, bool speculative,
					ParamExternData *prm)
{
	int			dno;
	...
	estate = (UPLpgSQL_execstate *) params->paramFetchArg;
	expr = (UPLpgSQL_expr *) params->parserSetupArg;
	 ...
	 */
	if (datum->dtype == UPLPGSQL_DTYPE_VAR)
		prm->value = MakeExpandedObjectReadOnly(prm->value,
												prm->isnull,
												((UPLpgSQL_var *) datum)->datatype->typlen);
	else if (datum->dtype == UPLPGSQL_DTYPE_REC)
		prm->value = MakeExpandedObjectReadOnly(prm->value,
												prm->isnull,
												-1);
...
/*
 * uplpgsql_param_compile		paramCompile callback for plpgsql parameters
 */
static void
uplpgsql_param_compile(ParamListInfo params, Param *param,
					  ExprState *state,
					  Datum *resv, bool *resnull)
{
	UPLpgSQL_execstate *estate;
	...
	if (datum->dtype == UPLPGSQL_DTYPE_VAR)
	{
		bool		isvarlena = (((UPLpgSQL_var *) datum)->datatype->typlen == -1);

		if (isvarlena && dno == expr->target_param && expr->expr_simple_expr)
			scratch.d.cparam.paramfunc = uplpgsql_param_eval_var_check;
		else if (isvarlena)
			scratch.d.cparam.paramfunc = uplpgsql_param_eval_var_ro;
		else
			scratch.d.cparam.paramfunc = uplpgsql_param_eval_var;
	}
	else if (datum->dtype == UPLPGSQL_DTYPE_RECFIELD)
		scratch.d.cparam.paramfunc = uplpgsql_param_eval_recfield;
	else if (datum->dtype == UPLPGSQL_DTYPE_PROMISE)
	{
		if (((UPLpgSQL_var *) datum)->datatype->typlen == -1)
			scratch.d.cparam.paramfunc = uplpgsql_param_eval_generic_ro;
		else
			scratch.d.cparam.paramfunc = uplpgsql_param_eval_generic;
	}
	else if (datum->dtype == UPLPGSQL_DTYPE_REC)
		scratch.d.cparam.paramfunc = uplpgsql_param_eval_generic_ro;
	else
	 ...
	 * pointers to the UPLpgSQL_expr as well as this specific Param, to support
	 * uplpgsql_param_eval_var_check().
	 */
...
/*
 * uplpgsql_param_eval_var_check		evaluation of EEOP_PARAM_CALLBACK step
 *
 ...
 * what would be done by uplpgsql_param_eval_var_ro, which is what we'd
 * have used otherwise.
 */
static void
uplpgsql_param_eval_var_check(ExprState *state, ExprEvalStep *op,
							 ExprContext *econtext)
{
	...
	/* now we can access the target datum */
	var = (UPLpgSQL_var *) estate->datums[dno];
	...
	{
		UPLpgSQL_expr *expr = (UPLpgSQL_expr *) op->d.cparam.paramarg;
		 ...
		 */
		if (expr->expr_rwopt == UPLPGSQL_RWOPT_UNKNOWN)
		...
		{
			case UPLPGSQL_RWOPT_UNKNOWN:
				Assert(false);
				break;
			case UPLPGSQL_RWOPT_NOPE:
				/* Force the value to read-only in all future executions */
				op->d.cparam.paramfunc = uplpgsql_param_eval_var_ro;
				uplpgsql_param_eval_var_ro(state, op, econtext);
				break;
			case UPLPGSQL_RWOPT_TRANSFER:
				...
				op->d.cparam.paramfunc = uplpgsql_param_eval_var_transfer;
				uplpgsql_param_eval_var_transfer(state, op, econtext);
				break;
			case UPLPGSQL_RWOPT_INPLACE:
				if (param == expr->expr_rw_param)
				{
					/* When the value is read/write, deliver it as-is */
					op->d.cparam.paramfunc = uplpgsql_param_eval_var;
					uplpgsql_param_eval_var(state, op, econtext);
				}
				else
				{
...
/*
 * uplpgsql_param_eval_var_transfer		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 ...
 */
static void
uplpgsql_param_eval_var_transfer(ExprState *state, ExprEvalStep *op,
								ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
...
/*
 * uplpgsql_param_eval_var		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we do not need to invoke MakeExpandedObjectReadOnly.
 */
static void
uplpgsql_param_eval_var(ExprState *state, ExprEvalStep *op,
					   ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
...
/*
 * uplpgsql_param_eval_var_ro		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we need to invoke MakeExpandedObjectReadOnly.
 */
static void
uplpgsql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
						  ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
...
/*
 * uplpgsql_param_eval_recfield		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_RECFIELD variables, for which
 * we never need to invoke MakeExpandedObjectReadOnly.
 */
static void
uplpgsql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
							ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
	int			dno = op->d.cparam.paramid - 1;
	UPLpgSQL_recfield *recfield;
	UPLpgSQL_rec *rec;
	ExpandedRecordHeader *erh;
	...
	/* now we can access the target datum */
	recfield = (UPLpgSQL_recfield *) estate->datums[dno];
	Assert(recfield->dtype == UPLPGSQL_DTYPE_RECFIELD);
...
/*
 * uplpgsql_param_eval_generic		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This handles all variable types, but assumes we do not need to invoke
 * MakeExpandedObjectReadOnly.
 */
static void
uplpgsql_param_eval_generic(ExprState *state, ExprEvalStep *op,
						   ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
...
/*
 * uplpgsql_param_eval_generic_ro	evaluation of EEOP_PARAM_CALLBACK step
 *
 ...
 */
static void
uplpgsql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
							  ExprContext *econtext)
{
	ParamListInfo params;
	UPLpgSQL_execstate *estate;
 ...
 */
void
exec_move_row(UPLpgSQL_execstate *estate,
			  UPLpgSQL_variable *target,
			  HeapTuple tup, TupleDesc tupdesc)
{
...
/*
 * Verify that a UPLpgSQL_rec's rectypeid is up-to-date.
 */
static void
revalidate_rectypeid(UPLpgSQL_rec *rec)
{
	UPLpgSQL_type *typ = rec->datatype;
...
make_expanded_record_for_rec(UPLpgSQL_execstate *estate,
							 UPLpgSQL_rec *rec,
							 TupleDesc srctupdesc,
							 ExpandedRecordHeader *srcerh)
{
 ...
 */
static void
exec_move_row_from_fields(UPLpgSQL_execstate *estate,
						  UPLpgSQL_variable *target,
						  ExpandedRecordHeader *newerh,
						  Datum *values, bool *nulls,
						  TupleDesc tupdesc)
{
	...
	if (tupdesc != NULL)
	{
		if (uplpgsql_extra_errors & UPLPGSQL_XCHECK_STRICTMULTIASSIGNMENT)
			strict_multiassignment_level = ERROR;
		else if (uplpgsql_extra_warnings & UPLPGSQL_XCHECK_STRICTMULTIASSIGNMENT)
	 ...
	 * the row variable.
	 */
	if (target->dtype == UPLPGSQL_DTYPE_ROW)
	{
		UPLpgSQL_row *row = (UPLpgSQL_row *) target;
		...
		for (fnum = 0; fnum < row->nfields; fnum++)
		{
			...
			var = (UPLpgSQL_var *) (estate->datums[row->varnos[fnum]]);
			...
			exec_assign_value(estate, (UPLpgSQL_datum *) var,
 ...
 */
static HeapTuple
make_tuple_from_row(UPLpgSQL_execstate *estate,
					UPLpgSQL_row *row,
					TupleDesc tupdesc)
{
 ...
 */
static void
exec_move_row_from_datum(UPLpgSQL_execstate *estate,
						 UPLpgSQL_variable *target,
						 Datum value)
{
 ...
 */
static void
instantiate_empty_record_variable(UPLpgSQL_execstate *estate, UPLpgSQL_rec *rec)
{
 ...
 */
static char *
convert_value_to_string(UPLpgSQL_execstate *estate, Datum value, Oid valtype)
{
	char	   *result;
	MemoryContext oldcontext;
	Oid			typoutput;
 ...
 */
static inline Datum
exec_cast_value(UPLpgSQL_execstate *estate,
				Datum value, bool *isnull,
				Oid valtype, int32 valtypmod,
				Oid reqtype, int32 reqtypmod)
{
 ...
 */
static Datum
do_cast_value(UPLpgSQL_execstate *estate,
			  Datum value, bool *isnull,
			  Oid valtype, int32 valtypmod,
			  Oid reqtype, int32 reqtypmod)
{
	uplpgsql_CastHashEntry *cast_entry;
 ...
 *
 * Returns a uplpgsql_CastHashEntry if an expression has to be evaluated,
 ...
 * ----------
 */
static uplpgsql_CastHashEntry *
get_cast_hashentry(UPLpgSQL_execstate *estate,
				   Oid srctype, int32 srctypmod,
				   Oid dsttype, int32 dsttypmod)
{
	uplpgsql_CastHashKey cast_key;
	uplpgsql_CastHashEntry *cast_entry;
	uplpgsql_CastExprHashEntry *expr_entry;
	bool		found;
	...
	cast_entry = (uplpgsql_CastHashEntry *) hash_search(estate->cast_hash,
													   &cast_key,
													   HASH_ENTER, &found);
	if (!found)					/* initialize if new entry */
	{
		/* We need a second lookup to see if a cast_expr_hash entry exists */
		expr_entry = (uplpgsql_CastExprHashEntry *) hash_search(cast_expr_hash,
															   &cast_key,
															   HASH_ENTER,
															   &found);
 ...
 */
static void
exec_simple_check_plan(UPLpgSQL_execstate *estate, UPLpgSQL_expr *expr)
{
	List	   *plansources;
 ...
 */
static bool
exec_is_simple_query(UPLpgSQL_expr *expr)
{
	List	   *plansources;
	CachedPlanSource *plansource;
	Query	   *query;
 ...
 */
static void
exec_save_simple_expr(UPLpgSQL_expr *expr, CachedPlan *cplan)
{
	PlannedStmt *stmt;
	Plan	   *plan;
	Expr	   *tle_expr;
 ...
 */
static void
exec_check_rw_parameter(UPLpgSQL_expr *expr, int paramid)
{
	...
	/* Assume unsafe */
	expr->expr_rwopt = UPLPGSQL_RWOPT_NOPE;
	expr->expr_rw_param = NULL;
	...
	if (expr->target_is_local)
	{
		...
		if (context.count == 1)
		{
			expr->expr_rwopt = UPLPGSQL_RWOPT_TRANSFER;
	...
	if (OidIsValid(prosupport))
	{
		...
		expr->expr_rwopt = UPLPGSQL_RWOPT_INPLACE;
 ...
 */
static void
exec_check_assignable(UPLpgSQL_execstate *estate, int dno)
{
	UPLpgSQL_datum *datum;
	...
	switch (datum->dtype)
	{
		...
		case UPLPGSQL_DTYPE_RECFIELD:
			...
			exec_check_assignable(estate,
								  ((UPLpgSQL_recfield *) datum)->recparentno);
			break;
		default:
			elog(ERROR, "unrecognized dtype: %d", datum->dtype);
			break;
...
exec_set_found(UPLpgSQL_execstate *estate, bool state)
{
	...
	var = (UPLpgSQL_var *) (estate->datums[estate->found_varno]);
...
/*
 * uplpgsql_set_sqlstate --- update the user-declared SQLSTATE variable
 *
 * SQL/PSM allows declaring a SQLSTATE char(5) variable that is automatically
 * updated by the runtime after FETCH (to '02000' on NOT FOUND), at block
 * entry (to '00000'), and when exception handlers fire.
 */
void
uplpgsql_set_sqlstate(UPLpgSQL_execstate *estate)
{
	UPLpgSQL_var *var;
	...
	var = (UPLpgSQL_var *) (estate->datums[estate->sqlstate_varno]);

	assign_text_var(estate, var,
					unpack_sql_state(estate->sqlerrcode));
}

/*
 * uplpgsql_create_econtext --- create an eval_econtext for the current function
 *
 ...
 */
void
uplpgsql_create_econtext(UPLpgSQL_execstate *estate)
{
	 ...
	 * required EState was already made by uplpgsql_inline_handler.  However,
...
/*
 * uplpgsql_destroy_econtext --- destroy function's econtext
 *
 * We check that it matches the top stack entry, and destroy the stack
 * entry along with the context.
 */
void
uplpgsql_destroy_econtext(UPLpgSQL_execstate *estate)
{
...
/*
 * uplpgsql_xact_cb --- post-transaction-commit-or-abort cleanup
 *
 * If a simple-expression EState was created in the current transaction,
 * it has to be cleaned up.  The same for the simple-expression resowner.
 */
void
uplpgsql_xact_cb(XactEvent event, void *arg)
{
	/*
...
/*
 * uplpgsql_subxact_cb --- post-subtransaction-commit-or-abort cleanup
 *
 ...
 */
void
uplpgsql_subxact_cb(SubXactEvent event, SubTransactionId mySubid,
 ...
 */
void
assign_simple_var(UPLpgSQL_execstate *estate, UPLpgSQL_var *var,
				  Datum newvalue, bool isnull, bool freeable)
{
	Assert(var->dtype == UPLPGSQL_DTYPE_VAR ||
 ...
 */
void
assign_text_var(UPLpgSQL_execstate *estate, UPLpgSQL_var *var, const char *str)
{
 ...
 */
static void
assign_record_var(UPLpgSQL_execstate *estate, UPLpgSQL_rec *rec,
				  ExpandedRecordHeader *erh)
{
	Assert(rec->dtype == UPLPGSQL_DTYPE_REC);
 ...
 */
static ParamListInfo
exec_eval_using_params(UPLpgSQL_execstate *estate, List *params)
{
	ParamListInfo paramLI;
	int			nargs;
	MemoryContext stmt_mcontext;
 ...
 */
Portal
exec_dynquery_with_params(UPLpgSQL_execstate *estate,
 ...
 */
static char *
format_expr_params(UPLpgSQL_execstate *estate,
				   const UPLpgSQL_expr *expr)
{
	int			paramno;
	int			dno;
	...
	while ((dno = bms_next_member(expr->paramnos, dno)) >= 0)
	{
		...
		curvar = (UPLpgSQL_var *) estate->datums[dno];

		exec_eval_datum(estate, (UPLpgSQL_datum *) curvar,
 ...
 */
static char *
format_preparedparamsdata(UPLpgSQL_execstate *estate,
						  ParamListInfo paramLI)
{
	int			paramno;
	StringInfoData paramstr;
