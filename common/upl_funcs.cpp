 ...
 *
 * Derived from PostgreSQL src/pl/plpgsql/src/pl_funcs.c; modifications are
 ...
 *
 * Block boundaries in the namespace chain are marked by UPLPGSQL_NSTYPE_LABEL
 * items.
 * ----------
 */
static UPLpgSQL_nsitem *ns_top = NULL;


/* ----------
 * uplpgsql_ns_init			Initialize namespace processing for a new function
 * ----------
 */
void
uplpgsql_ns_init(void)
{
	ns_top = NULL;
}
...
/* ----------
 * uplpgsql_ns_push			Create a new namespace level
 * ----------
 */
void
uplpgsql_ns_push(const char *label, UPLpgSQL_label_type label_type)
{
	if (label == NULL)
		label = "";
	uplpgsql_ns_additem(UPLPGSQL_NSTYPE_LABEL, (int) label_type, label);
}


/* ----------
 * uplpgsql_ns_pop			Pop entries back to (and including) the last label
 * ----------
 */
void
uplpgsql_ns_pop(void)
{
	Assert(ns_top != NULL);
	while (ns_top->itemtype != UPLPGSQL_NSTYPE_LABEL)
		ns_top = ns_top->prev;
	ns_top = ns_top->prev;
}
...
/* ----------
 * uplpgsql_ns_top			Fetch the current namespace chain end
 * ----------
 */
UPLpgSQL_nsitem *
uplpgsql_ns_top(void)
{
	return ns_top;
}
...
/* ----------
 * uplpgsql_ns_additem		Add an item to the current namespace chain
 * ----------
 */
void
uplpgsql_ns_additem(UPLpgSQL_nsitem_type itemtype, int itemno, const char *name)
{
	UPLpgSQL_nsitem *nse;
...
/* ----------
 * uplpgsql_ns_lookup		Lookup an identifier in the given namespace chain
 *
 ...
 */
UPLpgSQL_nsitem *
uplpgsql_ns_lookup(UPLpgSQL_nsitem *ns_cur, bool localmode,
				  ...
				  int *names_used)
{
	...
	while (ns_cur != NULL)
	{
		...
		for (nsitem = ns_cur;
			 nsitem->itemtype != UPLPGSQL_NSTYPE_LABEL;
			 nsitem = nsitem->prev)
		{
			if (strcmp(nsitem->name, name1) == 0)
			{
				if (name2 == NULL ||
					nsitem->itemtype != UPLPGSQL_NSTYPE_VAR)
				{
					if (names_used)
						*names_used = 1;
					return nsitem;
...
/* ----------
 * uplpgsql_ns_lookup_label		Lookup a label in the given namespace chain
 * ----------
 */
UPLpgSQL_nsitem *
uplpgsql_ns_lookup_label(UPLpgSQL_nsitem *ns_cur, const char *name)
{
	while (ns_cur != NULL)
	{
		if (ns_cur->itemtype == UPLPGSQL_NSTYPE_LABEL &&
...
/* ----------
 * uplpgsql_ns_find_nearest_loop		Find innermost loop label in namespace chain
 * ----------
 */
UPLpgSQL_nsitem *
uplpgsql_ns_find_nearest_loop(UPLpgSQL_nsitem *ns_cur)
{
	while (ns_cur != NULL)
	{
		if (ns_cur->itemtype == UPLPGSQL_NSTYPE_LABEL &&
			ns_cur->itemno == UPLPGSQL_LABEL_LOOP)
			return ns_cur;
		ns_cur = ns_cur->prev;
	}
 ...
 */
const char *
uplpgsql_stmt_typename(UPLpgSQL_stmt *stmt)
{
	switch (stmt->cmd_type)
	{
		...
		case UPLPGSQL_STMT_EXIT:
			return ((UPLpgSQL_stmt_exit *) stmt)->is_exit ? "EXIT" : "CONTINUE";
 ...
 */
const char *
uplpgsql_getdiag_kindname(UPLpgSQL_getdiag_kind kind)
{
	switch (kind)
	{
 ...
 * uplpgsql_statement_tree_walker().  The expression callback can be a
 * no-op if no per-expression behavior is needed.
 **********************************************************************/
typedef void (*uplpgsql_stmt_walker_callback) (UPLpgSQL_stmt *stmt,
											  void *context);
typedef void (*uplpgsql_expr_walker_callback) (UPLpgSQL_expr *expr,
											  void *context);
 ...
 */
#define uplpgsql_statement_tree_walker(s, sw, ew, c) \
	uplpgsql_statement_tree_walker_impl(s, (uplpgsql_stmt_walker_callback) (sw), \
									   (uplpgsql_expr_walker_callback) (ew), c)

static void
uplpgsql_statement_tree_walker_impl(UPLpgSQL_stmt *stmt,
								   uplpgsql_stmt_walker_callback stmt_callback,
								   uplpgsql_expr_walker_callback expr_callback,
								   void *context)
{
#define S_WALK(st) stmt_callback(st, context)
#define E_WALK(ex) expr_callback(ex, context)
#define S_LIST_WALK(lst) foreach_ptr(UPLpgSQL_stmt, st, lst) S_WALK(st)
#define E_LIST_WALK(lst) foreach_ptr(UPLpgSQL_expr, ex, lst) E_WALK(ex)

	switch (stmt->cmd_type)
	{
		case UPLPGSQL_STMT_BLOCK:
			{
				UPLpgSQL_stmt_block *bstmt = (UPLpgSQL_stmt_block *) stmt;

				S_LIST_WALK(bstmt->body);
				if (bstmt->exceptions)
				{
					foreach_ptr(UPLpgSQL_exception, exc, bstmt->exceptions->exc_list)
					{
		...
		case UPLPGSQL_STMT_ASSIGN:
			{
				UPLpgSQL_stmt_assign *astmt = (UPLpgSQL_stmt_assign *) stmt;
		...
		case UPLPGSQL_STMT_IF:
			{
				...
				foreach_ptr(UPLpgSQL_if_elsif, elif, ifstmt->elsif_list)
				{
					E_WALK(elif->cond);
		...
		case UPLPGSQL_STMT_CASE:
			{
				UPLpgSQL_stmt_case *cstmt = (UPLpgSQL_stmt_case *) stmt;

				E_WALK(cstmt->t_expr);
				foreach_ptr(UPLpgSQL_case_when, cwt, cstmt->case_when_list)
				{
					E_WALK(cwt->expr);
		...
		case UPLPGSQL_STMT_LOOP:
			{
				UPLpgSQL_stmt_loop *lstmt = (UPLpgSQL_stmt_loop *) stmt;
		...
		case UPLPGSQL_STMT_WHILE:
			{
				UPLpgSQL_stmt_while *wstmt = (UPLpgSQL_stmt_while *) stmt;
		...
		case UPLPGSQL_STMT_FORI:
			{
				UPLpgSQL_stmt_fori *fori = (UPLpgSQL_stmt_fori *) stmt;
		...
		case UPLPGSQL_STMT_FORS:
			{
				UPLpgSQL_stmt_fors *fors = (UPLpgSQL_stmt_fors *) stmt;
		...
		case UPLPGSQL_STMT_FORC:
			{
				UPLpgSQL_stmt_forc *forc = (UPLpgSQL_stmt_forc *) stmt;
		...
		case UPLPGSQL_STMT_FOREACH_A:
			{
				UPLpgSQL_stmt_foreach_a *fstmt = (UPLpgSQL_stmt_foreach_a *) stmt;
		...
		case UPLPGSQL_STMT_EXIT:
			{
				UPLpgSQL_stmt_exit *estmt = (UPLpgSQL_stmt_exit *) stmt;
		...
		case UPLPGSQL_STMT_RETURN:
			{
				UPLpgSQL_stmt_return *rstmt = (UPLpgSQL_stmt_return *) stmt;
		...
		case UPLPGSQL_STMT_RETURN_NEXT:
			{
				UPLpgSQL_stmt_return_next *rstmt = (UPLpgSQL_stmt_return_next *) stmt;
		...
		case UPLPGSQL_STMT_RETURN_QUERY:
			{
				UPLpgSQL_stmt_return_query *rstmt = (UPLpgSQL_stmt_return_query *) stmt;
		...
		case UPLPGSQL_STMT_RAISE:
			{
				UPLpgSQL_stmt_raise *rstmt = (UPLpgSQL_stmt_raise *) stmt;

				E_LIST_WALK(rstmt->params);
				foreach_ptr(UPLpgSQL_raise_option, opt, rstmt->options)
				{
					E_WALK(opt->expr);
				}
				break;
		...
		case UPLPGSQL_STMT_ASSERT:
			{
				UPLpgSQL_stmt_assert *astmt = (UPLpgSQL_stmt_assert *) stmt;
		...
		case UPLPGSQL_STMT_EXECSQL:
			{
				UPLpgSQL_stmt_execsql *xstmt = (UPLpgSQL_stmt_execsql *) stmt;
		...
		case UPLPGSQL_STMT_DYNEXECUTE:
			{
				UPLpgSQL_stmt_dynexecute *dstmt = (UPLpgSQL_stmt_dynexecute *) stmt;
		...
		case UPLPGSQL_STMT_DYNFORS:
			{
				UPLpgSQL_stmt_dynfors *dstmt = (UPLpgSQL_stmt_dynfors *) stmt;
		...
		case UPLPGSQL_STMT_OPEN:
			{
				UPLpgSQL_stmt_open *ostmt = (UPLpgSQL_stmt_open *) stmt;
		...
		case UPLPGSQL_STMT_FETCH:
			{
				UPLpgSQL_stmt_fetch *fstmt = (UPLpgSQL_stmt_fetch *) stmt;
		...
		case UPLPGSQL_STMT_PERFORM:
			{
				UPLpgSQL_stmt_perform *pstmt = (UPLpgSQL_stmt_perform *) stmt;
		...
		case UPLPGSQL_STMT_CALL:
			{
				UPLpgSQL_stmt_call *cstmt = (UPLpgSQL_stmt_call *) stmt;
 ...
 **********************************************************************/
static void mark_stmt(UPLpgSQL_stmt *stmt, Bitmapset *local_dnos);
static void mark_expr(UPLpgSQL_expr *expr, Bitmapset *local_dnos);

static void
mark_stmt(UPLpgSQL_stmt *stmt, Bitmapset *local_dnos)
{
	if (stmt == NULL)
		return;
	if (stmt->cmd_type == UPLPGSQL_STMT_BLOCK)
	{
		UPLpgSQL_stmt_block *block = (UPLpgSQL_stmt_block *) stmt;
		...
		if (block->exceptions)
		{
			 ...
			 */
			uplpgsql_statement_tree_walker(stmt, mark_stmt, mark_expr, NULL);
		}
		else
		{
			/*
			...
			uplpgsql_statement_tree_walker(stmt, mark_stmt, mark_expr,
										  local_dnos);
			bms_free(local_dnos);
		}
	}
	else
		uplpgsql_statement_tree_walker(stmt, mark_stmt, mark_expr, local_dnos);
}

static void
mark_expr(UPLpgSQL_expr *expr, Bitmapset *local_dnos)
{
	/*
...
void
uplpgsql_mark_local_assignment_targets(UPLpgSQL_function *func)
{
	...
	mark_stmt((UPLpgSQL_stmt *) func->action, local_dnos);
	bms_free(local_dnos);
}
 ...
 *
 * This code only needs to deal with cleaning up UPLpgSQL_expr nodes,
 ...
 **********************************************************************/
static void free_stmt(UPLpgSQL_stmt *stmt, void *context);
static void free_expr(UPLpgSQL_expr *expr, void *context);

static void
free_stmt(UPLpgSQL_stmt *stmt, void *context)
{
	if (stmt == NULL)
		return;
	uplpgsql_statement_tree_walker(stmt, free_stmt, free_expr, NULL);
}

static void
free_expr(UPLpgSQL_expr *expr, void *context)
{
	if (expr && expr->plan)
	{
...
void
uplpgsql_free_function_memory(UPLpgSQL_function *func)
{
	int			i;
	...
	for (i = 0; i < func->ndatums; i++)
	{
		...
		switch (d->dtype)
		{
			...
			case UPLPGSQL_DTYPE_PROMISE:
				{
					UPLpgSQL_var *var = (UPLpgSQL_var *) d;
			...
			case UPLPGSQL_DTYPE_REC:
				{
					UPLpgSQL_rec *rec = (UPLpgSQL_rec *) d;
	...
	free_stmt((UPLpgSQL_stmt *) func->action, NULL);
	func->action = NULL;

	/*
	 * And finally, release all memory except the UPLpgSQL_function struct
...
void
uplpgsql_delete_callback(CachedFunction *cfunc)
{
	uplpgsql_free_function_memory((UPLpgSQL_function *) cfunc);
}
 ...
 *
 * Sadly, there doesn't seem to be any way to let uplpgsql_statement_tree_walker
 * bear some of the burden for this.
 **********************************************************************/
static int	dump_indent;

static void dump_ind(void);
static void dump_stmt(UPLpgSQL_stmt *stmt);
static void dump_block(UPLpgSQL_stmt_block *block);
static void dump_assign(UPLpgSQL_stmt_assign *stmt);
static void dump_if(UPLpgSQL_stmt_if *stmt);
static void dump_case(UPLpgSQL_stmt_case *stmt);
static void dump_loop(UPLpgSQL_stmt_loop *stmt);
static void dump_while(UPLpgSQL_stmt_while *stmt);
static void dump_fori(UPLpgSQL_stmt_fori *stmt);
static void dump_fors(UPLpgSQL_stmt_fors *stmt);
static void dump_forc(UPLpgSQL_stmt_forc *stmt);
static void dump_foreach_a(UPLpgSQL_stmt_foreach_a *stmt);
static void dump_exit(UPLpgSQL_stmt_exit *stmt);
static void dump_return(UPLpgSQL_stmt_return *stmt);
static void dump_return_next(UPLpgSQL_stmt_return_next *stmt);
static void dump_return_query(UPLpgSQL_stmt_return_query *stmt);
static void dump_raise(UPLpgSQL_stmt_raise *stmt);
static void dump_assert(UPLpgSQL_stmt_assert *stmt);
static void dump_execsql(UPLpgSQL_stmt_execsql *stmt);
static void dump_dynexecute(UPLpgSQL_stmt_dynexecute *stmt);
static void dump_dynfors(UPLpgSQL_stmt_dynfors *stmt);
static void dump_getdiag(UPLpgSQL_stmt_getdiag *stmt);
static void dump_open(UPLpgSQL_stmt_open *stmt);
static void dump_fetch(UPLpgSQL_stmt_fetch *stmt);
static void dump_cursor_direction(UPLpgSQL_stmt_fetch *stmt);
static void dump_close(UPLpgSQL_stmt_close *stmt);
static void dump_perform(UPLpgSQL_stmt_perform *stmt);
static void dump_call(UPLpgSQL_stmt_call *stmt);
static void dump_commit(UPLpgSQL_stmt_commit *stmt);
static void dump_rollback(UPLpgSQL_stmt_rollback *stmt);
static void dump_expr(UPLpgSQL_expr *expr);
...
static void
dump_stmt(UPLpgSQL_stmt *stmt)
{
	printf("%3d:", stmt->lineno);
	switch (stmt->cmd_type)
	{
		case UPLPGSQL_STMT_BLOCK:
			dump_block((UPLpgSQL_stmt_block *) stmt);
			break;
		case UPLPGSQL_STMT_ASSIGN:
			dump_assign((UPLpgSQL_stmt_assign *) stmt);
			break;
		case UPLPGSQL_STMT_IF:
		...
		case UPLPGSQL_STMT_CASE:
			...
			break;
		case UPLPGSQL_STMT_LOOP:
			dump_loop((UPLpgSQL_stmt_loop *) stmt);
			break;
		case UPLPGSQL_STMT_WHILE:
			dump_while((UPLpgSQL_stmt_while *) stmt);
			break;
		case UPLPGSQL_STMT_FORI:
			dump_fori((UPLpgSQL_stmt_fori *) stmt);
			break;
		case UPLPGSQL_STMT_FORS:
			dump_fors((UPLpgSQL_stmt_fors *) stmt);
			break;
		case UPLPGSQL_STMT_FORC:
			dump_forc((UPLpgSQL_stmt_forc *) stmt);
			break;
		case UPLPGSQL_STMT_FOREACH_A:
			dump_foreach_a((UPLpgSQL_stmt_foreach_a *) stmt);
			break;
		case UPLPGSQL_STMT_EXIT:
			dump_exit((UPLpgSQL_stmt_exit *) stmt);
			break;
		case UPLPGSQL_STMT_RETURN:
			dump_return((UPLpgSQL_stmt_return *) stmt);
			break;
		case UPLPGSQL_STMT_RETURN_NEXT:
			dump_return_next((UPLpgSQL_stmt_return_next *) stmt);
			break;
		case UPLPGSQL_STMT_RETURN_QUERY:
			dump_return_query((UPLpgSQL_stmt_return_query *) stmt);
			break;
		case UPLPGSQL_STMT_RAISE:
			dump_raise((UPLpgSQL_stmt_raise *) stmt);
			break;
		case UPLPGSQL_STMT_ASSERT:
			dump_assert((UPLpgSQL_stmt_assert *) stmt);
			break;
		case UPLPGSQL_STMT_EXECSQL:
			dump_execsql((UPLpgSQL_stmt_execsql *) stmt);
			break;
		case UPLPGSQL_STMT_DYNEXECUTE:
			dump_dynexecute((UPLpgSQL_stmt_dynexecute *) stmt);
			break;
		case UPLPGSQL_STMT_DYNFORS:
			dump_dynfors((UPLpgSQL_stmt_dynfors *) stmt);
			break;
		case UPLPGSQL_STMT_GETDIAG:
			dump_getdiag((UPLpgSQL_stmt_getdiag *) stmt);
			break;
		case UPLPGSQL_STMT_OPEN:
			dump_open((UPLpgSQL_stmt_open *) stmt);
			break;
		case UPLPGSQL_STMT_FETCH:
			dump_fetch((UPLpgSQL_stmt_fetch *) stmt);
			break;
		case UPLPGSQL_STMT_CLOSE:
			dump_close((UPLpgSQL_stmt_close *) stmt);
			break;
		case UPLPGSQL_STMT_PERFORM:
			dump_perform((UPLpgSQL_stmt_perform *) stmt);
			break;
		case UPLPGSQL_STMT_CALL:
			dump_call((UPLpgSQL_stmt_call *) stmt);
			break;
		case UPLPGSQL_STMT_COMMIT:
			dump_commit((UPLpgSQL_stmt_commit *) stmt);
			break;
		case UPLPGSQL_STMT_ROLLBACK:
			dump_rollback((UPLpgSQL_stmt_rollback *) stmt);
			break;
		default:
			elog(ERROR, "unrecognized cmd_type: %d", stmt->cmd_type);
			break;
...
static void
dump_block(UPLpgSQL_stmt_block *block)
{
	char	   *name;
...
static void
dump_cursor_direction(UPLpgSQL_stmt_fetch *stmt)
{
	dump_indent += 2;
	dump_ind();
...
void
uplpgsql_dumptree(UPLpgSQL_function *func)
{
	int			i;
	UPLpgSQL_datum *d;
	...
	for (i = 0; i < func->ndatums; i++)
	{
		...
		switch (d->dtype)
		{
			...
			case UPLPGSQL_DTYPE_ROW:
				{
					UPLpgSQL_row *row = (UPLpgSQL_row *) d;
			...
			case UPLPGSQL_DTYPE_REC:
				printf("REC %-16s typoid %u\n",
					   ((UPLpgSQL_rec *) d)->refname,
					   ((UPLpgSQL_rec *) d)->rectypeid);
				if (((UPLpgSQL_rec *) d)->isconst)
					printf("                                  CONSTANT\n");
				if (((UPLpgSQL_rec *) d)->notnull)
					printf("                                  NOT NULL\n");
				if (((UPLpgSQL_rec *) d)->default_val != NULL)
				{
					printf("                                  DEFAULT ");
					dump_expr(((UPLpgSQL_rec *) d)->default_val);
					printf("\n");
				}
				break;
			case UPLPGSQL_DTYPE_RECFIELD:
				printf("RECFIELD %-16s of REC %d\n",
					   ((UPLpgSQL_recfield *) d)->fieldname,
					   ((UPLpgSQL_recfield *) d)->recparentno);
				break;
			default:
