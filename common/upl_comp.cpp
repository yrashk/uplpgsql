 ...
 */
void
uplpgsql_parser_setup(struct ParseState *pstate, UPLpgSQL_expr *expr)
{
	pstate->p_pre_columnref_hook = uplpgsql_pre_column_ref;
	pstate->p_post_columnref_hook = uplpgsql_post_column_ref;
	pstate->p_paramref_hook = uplpgsql_param_ref;
	/* no need to use p_coerce_param_hook */
	pstate->p_ref_hook_state = expr;
}

/*
 * uplpgsql_pre_column_ref		parser callback before parsing a ColumnRef
 */
static Node *
uplpgsql_pre_column_ref(ParseState *pstate, ColumnRef *cref)
{
	UPLpgSQL_expr *expr = (UPLpgSQL_expr *) pstate->p_ref_hook_state;

	if (expr->func->resolve_option == UPLPGSQL_RESOLVE_VARIABLE)
...
/*
 * uplpgsql_post_column_ref		parser callback after parsing a ColumnRef
 */
static Node *
uplpgsql_post_column_ref(ParseState *pstate, ColumnRef *cref, Node *var)
{
	UPLpgSQL_expr *expr = (UPLpgSQL_expr *) pstate->p_ref_hook_state;
	Node	   *myvar;
	...
	if (expr->func->resolve_option == UPLPGSQL_RESOLVE_COLUMN && var != NULL)
...
/*
 * uplpgsql_param_ref		parser callback for ParamRefs ($n symbols)
 */
static Node *
uplpgsql_param_ref(ParseState *pstate, ParamRef *pref)
{
	UPLpgSQL_expr *expr = (UPLpgSQL_expr *) pstate->p_ref_hook_state;
	char		pname[32];
	UPLpgSQL_nsitem *nse;
	...
	nse = uplpgsql_ns_lookup(expr->ns, false,
							pname, NULL, NULL,
							NULL);
 ...
 */
static Node *
resolve_column_ref(ParseState *pstate, UPLpgSQL_expr *expr,
				   ColumnRef *cref, bool error_if_no_field)
{
	...
	switch (nse->itemtype)
	{
		...
		case UPLPGSQL_NSTYPE_REC:
			...
			if (nnames == nnames_field)
			{
				...
				UPLpgSQL_rec *rec = (UPLpgSQL_rec *) estate->datums[nse->itemno];
				int			i;
				...
				while (i >= 0)
				{
					UPLpgSQL_recfield *fld = (UPLpgSQL_recfield *) estate->datums[i];

					Assert(fld->dtype == UPLPGSQL_DTYPE_RECFIELD &&
				 ...
				 * source text.  However, uplpgsql_yylex will not build such a
 ...
 */
static Node *
make_datum_param(UPLpgSQL_expr *expr, int dno, int location)
{
	UPLpgSQL_execstate *estate;
	...
	param->paramid = dno + 1;
	uplpgsql_exec_get_datum_type_info(estate,
									 datum,
									 &param->paramtype,
									 &param->paramtypmod,
									 &param->paramcollid);
...
/* ----------
 * uplpgsql_parse_word		The scanner calls this to postparse
 ...
 * and the uplpgsql_IdentifierLookup context permits it.
 *
 ...
 */
bool
uplpgsql_parse_word(char *word1, const char *yytxt, bool lookup,
				   PLwdatum *wdatum, PLword *word)
{
	 ...
	 */
	if (lookup && uplpgsql_IdentifierLookup == IDENTIFIER_LOOKUP_NORMAL)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
		ns = uplpgsql_ns_lookup(uplpgsql_ns_top(), false,
							   word1, NULL, NULL,
							   NULL);
		...
		if (ns != NULL)
		{
			switch (ns->itemtype)
			{
				case UPLPGSQL_NSTYPE_VAR:
				case UPLPGSQL_NSTYPE_REC:
					wdatum->datum = uplpgsql_Datums[ns->itemno];
					wdatum->ident = word1;
				...
				default:
					/* uplpgsql_ns_lookup should never return anything else */
...
/* ----------
 * uplpgsql_parse_dblword		Same lookup for two words
 *					separated by a dot.
 * ----------
 */
bool
uplpgsql_parse_dblword(char *word1, char *word2,
					  PLwdatum *wdatum, PLcword *cword)
{
	 ...
	 */
	if (uplpgsql_IdentifierLookup != IDENTIFIER_LOOKUP_DECLARE)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
...
/* ----------
 * uplpgsql_parse_tripword		Same lookup for three words
 *					separated by dots.
 * ----------
 */
bool
uplpgsql_parse_tripword(char *word1, char *word2, char *word3,
					   PLwdatum *wdatum, PLcword *cword)
{
	UPLpgSQL_nsitem *ns;
	List	   *idents;
...
/* ----------
 * uplpgsql_parse_wordtype	The scanner found word%TYPE. word should be
 ...
 */
UPLpgSQL_type *
uplpgsql_parse_wordtype(char *ident)
{
	UPLpgSQL_nsitem *nse;
	 ...
	 * Do a lookup in the current namespace stack
	 */
	nse = uplpgsql_ns_lookup(uplpgsql_ns_top(), false,
							ident, NULL, NULL,
							NULL);
	...
	if (nse != NULL)
	{
		...
		{
			case UPLPGSQL_NSTYPE_VAR:
				return ((UPLpgSQL_var *) (uplpgsql_Datums[nse->itemno]))->datatype;
			case UPLPGSQL_NSTYPE_REC:
				return ((UPLpgSQL_rec *) (uplpgsql_Datums[nse->itemno]))->datatype;
			default:
				break;
		}
	}
...
/* ----------
 * uplpgsql_parse_cwordtype		Same lookup for compositeword%TYPE
 *
 ...
 */
UPLpgSQL_type *
uplpgsql_parse_cwordtype(List *idents)
{
	UPLpgSQL_type *dtype = NULL;
	UPLpgSQL_nsitem *nse;
	int			nnames;
	...
	if (list_length(idents) == 2)
	{
		...
		if (nse != NULL && nse->itemtype == UPLPGSQL_NSTYPE_VAR)
		{
			/* Block-qualified reference to scalar variable. */
			dtype = ((UPLpgSQL_var *) (uplpgsql_Datums[nse->itemno]))->datatype;
			goto done;
		}
		else if (nse != NULL && nse->itemtype == UPLPGSQL_NSTYPE_REC &&
				 nnames == 2)
		{
			/* Block-qualified reference to record variable. */
			dtype = ((UPLpgSQL_rec *) (uplpgsql_Datums[nse->itemno]))->datatype;
			goto done;
		}
...
/* ----------
 * uplpgsql_parse_wordrowtype		Scanner found word%ROWTYPE.
 *					So word must be a table name.
 * ----------
 */
UPLpgSQL_type *
uplpgsql_parse_wordrowtype(char *ident)
{
	Oid			classOid;
	Oid			typOid;
	TypeName   *typName;
	...
	return uplpgsql_build_datatype(typOid, -1, InvalidOid, typName);
}

/* ----------
 * uplpgsql_parse_cwordrowtype		Scanner found compositeword%ROWTYPE.
 *			So word must be a namespace qualified table name.
 * ----------
 */
UPLpgSQL_type *
uplpgsql_parse_cwordrowtype(List *idents)
{
	Oid			classOid;
	Oid			typOid;
	RangeVar   *relvar;
...
/*
 * uplpgsql_build_variable - build a datum-array entry of a given
 * datatype
 *
 * The returned struct may be a UPLpgSQL_var or UPLpgSQL_rec
 * depending on the given datatype, and is allocated via
 * palloc.  The struct is automatically added to the current datum
 * array, and optionally to the current namespace.
 */
UPLpgSQL_variable *
uplpgsql_build_variable(const char *refname, int lineno, UPLpgSQL_type *dtype,
					   bool add2namespace)
{
	UPLpgSQL_variable *result;

	switch (dtype->ttype)
	{
		case UPLPGSQL_TTYPE_SCALAR:
			{
				/* Ordinary scalar datatype */
				UPLpgSQL_var *var;

				var = palloc0_object(UPLpgSQL_var);
				var->dtype = UPLPGSQL_DTYPE_VAR;
				...
				uplpgsql_adddatum((UPLpgSQL_datum *) var);
				if (add2namespace)
					uplpgsql_ns_additem(UPLPGSQL_NSTYPE_VAR,
									   var->dno,
									   refname);
				result = (UPLpgSQL_variable *) var;
				break;
			}
		case UPLPGSQL_TTYPE_REC:
			{
				/* Composite type -- build a record variable */
				UPLpgSQL_rec *rec;

				rec = uplpgsql_build_record(refname, lineno,
										   dtype, dtype->typoid,
										   add2namespace);
				result = (UPLpgSQL_variable *) rec;
				break;
			}
		case UPLPGSQL_TTYPE_PSEUDO:
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
 ...
 */
UPLpgSQL_rec *
uplpgsql_build_record(const char *refname, int lineno,
					 UPLpgSQL_type *dtype, Oid rectypeid,
					 bool add2namespace)
{
	UPLpgSQL_rec *rec;

	rec = palloc0_object(UPLpgSQL_rec);
	rec->dtype = UPLPGSQL_DTYPE_REC;
	...
	rec->firstfield = -1;
	rec->erh = NULL;
	uplpgsql_adddatum((UPLpgSQL_datum *) rec);
	if (add2namespace)
		uplpgsql_ns_additem(UPLPGSQL_NSTYPE_REC, rec->dno, rec->refname);
 ...
 */
static UPLpgSQL_row *
build_row_from_vars(UPLpgSQL_variable **vars, int numvars)
{
	UPLpgSQL_row *row;
	int			i;
	...
	for (i = 0; i < numvars; i++)
	{
		...
		switch (var->dtype)
		{
			...
			case UPLPGSQL_DTYPE_PROMISE:
				typoid = ((UPLpgSQL_var *) var)->datatype->typoid;
				typmod = ((UPLpgSQL_var *) var)->datatype->atttypmod;
				typcoll = ((UPLpgSQL_var *) var)->datatype->collation;
				break;

			case UPLPGSQL_DTYPE_REC:
				/* shouldn't need to revalidate rectypeid already... */
				typoid = ((UPLpgSQL_rec *) var)->rectypeid;
	...
	/*
	 * PG18+ requires TupleDescFinalize() to be called after manually
	 * populating a TupleDesc's attributes (it computes the compact_attrs
	 * offset caches and the firstNonCachedOffsetAttr/firstNonGuaranteedAttr
	 * fields).  Mirrors upstream plpgsql build_row().
	 */
 ...
 */
UPLpgSQL_recfield *
uplpgsql_build_recfield(UPLpgSQL_rec *rec, const char *fldname)
{
	...
	while (i >= 0)
	{
		UPLpgSQL_recfield *fld = (UPLpgSQL_recfield *) uplpgsql_Datums[i];
	...
	recfield = palloc0_object(UPLpgSQL_recfield);
	recfield->dtype = UPLPGSQL_DTYPE_RECFIELD;
	recfield->fieldname = pstrdup(fldname);
	recfield->recparentno = rec->dno;
	recfield->rectupledescid = INVALID_TUPLEDESC_IDENTIFIER;

	uplpgsql_adddatum((UPLpgSQL_datum *) recfield);
...
/*
 * uplpgsql_build_datatype
 *		Build UPLpgSQL_type struct given type OID, typmod, collation,
 ...
 */
UPLpgSQL_type *
uplpgsql_build_datatype(Oid typeOid, int32 typmod,
					   Oid collation, TypeName *origtypname)
{
	HeapTuple	typeTup;
	UPLpgSQL_type *typ;
...
/*
 * Utility subroutine to make a UPLpgSQL_type struct given a pg_type entry
 * and additional details (see comments for uplpgsql_build_datatype).
 */
			   ...
			   Oid collation, TypeName *origtypname)
{
	...
	typ = palloc_object(UPLpgSQL_type);
	...
	switch (typeStruct->typtype)
	{
		...
		case TYPTYPE_MULTIRANGE:
			typ->ttype = UPLPGSQL_TTYPE_SCALAR;
			break;
		case TYPTYPE_COMPOSITE:
			typ->ttype = UPLPGSQL_TTYPE_REC;
			break;
		case TYPTYPE_DOMAIN:
		...
		case TYPTYPE_PSEUDO:
			if (typ->typoid == RECORDOID)
				...
				typ->ttype = UPLPGSQL_TTYPE_PSEUDO;
			break;
		default:
	 ...
	 */
	if (typ->ttype == UPLPGSQL_TTYPE_REC && typ->typoid != RECORDOID)
	{
 ...
 */
UPLpgSQL_type *
uplpgsql_build_datatype_arrayof(UPLpgSQL_type *dtype)
{
	Oid			array_typeid;
	...
	return uplpgsql_build_datatype(array_typeid, dtype->atttypmod,
								  dtype->collation, NULL);
}

/*
 *	uplpgsql_recognize_err_condition
 ...
 */
int
uplpgsql_recognize_err_condition(const char *condname, bool allow_sqlstate)
{
	int			i;
...
/*
 * uplpgsql_parse_err_condition
 *		Generate UPLpgSQL_condition entry(s) for an exception condition name
 *
...
/* ----------
 * uplpgsql_start_datums			Initialize datum list at compile startup.
 * ----------
 */
static void
uplpgsql_start_datums(void)
{
	datums_alloc = 128;
	uplpgsql_nDatums = 0;
...
/* ----------
 * uplpgsql_adddatum			Add a variable, record or row
 *					to the compiler's datum list.
 * ----------
 */
void
uplpgsql_adddatum(UPLpgSQL_datum *newdatum)
{
	...
	uplpgsql_Datums[uplpgsql_nDatums++] = newdatum;
}

/* ----------
 * uplpgsql_finish_datums	Copy completed datum info into function struct.
 * ----------
 */
static void
uplpgsql_finish_datums(UPLpgSQL_function *function)
{
	Size		copiable_size = 0;
	int			i;

	function->ndatums = uplpgsql_nDatums;
	function->datums = palloc_array(UPLpgSQL_datum *, uplpgsql_nDatums);
	for (i = 0; i < uplpgsql_nDatums; i++)
	{
		function->datums[i] = uplpgsql_Datums[i];

		/* This must agree with copy_uplpgsql_datums on what is copiable */
		switch (function->datums[i]->dtype)
		{
			case UPLPGSQL_DTYPE_VAR:
			case UPLPGSQL_DTYPE_PROMISE:
				copiable_size += MAXALIGN(sizeof(UPLpgSQL_var));
				break;
			case UPLPGSQL_DTYPE_REC:
				copiable_size += MAXALIGN(sizeof(UPLpgSQL_rec));
				break;
			default:
				break;
		}
...
/* ----------
 * uplpgsql_add_initdatums		Make an array of the datum numbers of
 ...
 */
int
uplpgsql_add_initdatums(int **varnos)
{
	int			i;
	int			n = 0;
	 ...
	 */
	for (i = datums_last; i < uplpgsql_nDatums; i++)
	{
		switch (uplpgsql_Datums[i]->dtype)
		{
			case UPLPGSQL_DTYPE_VAR:
			case UPLPGSQL_DTYPE_REC:
				n++;
			...
			*varnos = palloc_array(int, n);
			...
			for (i = datums_last; i < uplpgsql_nDatums; i++)
			{
				switch (uplpgsql_Datums[i]->dtype)
				{
					case UPLPGSQL_DTYPE_VAR:
					case UPLPGSQL_DTYPE_REC:
						(*varnos)[n++] = uplpgsql_Datums[i]->dno;
						break;
